...
- **Lifecycle:** plugins implement the optional `init` / `start` / `destroy` hooks, driven by `kernel.bootstrap()` and shutdown
...
<Callout type="warn">
  ...
  only invokes `init` / `start` / `destroy` — rename `initialize` to `init` in
  the generated `src/index.ts` or the hook never runs.
</Callout>
