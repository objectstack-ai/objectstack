...
inbound** `X-Request-Id` and otherwise mints `req_<hex>`. The header name is configurable
...
outside the accepted character set is discarded and a fresh `req_<hex>` is minted instead.
