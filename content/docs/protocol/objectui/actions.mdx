...
{ name: 'task_complete', label: 'Complete', operation: 'update', patch: { status: 'done' },
  visible: 'record.status == "open"', undoable: true }
```
...
interface Action {
  ...
  operation?: 'update';         // Declarative single-record field write (rides the default script route)
  patch?: Record<string, unknown>; // Static field values for operation: 'update', merged under params
...
<Callout type="warn">
**The bound record is SPARSE — guard every read with `has()`.** Unlike a
validation or hook predicate, an action predicate is evaluated against whatever
record the client already fetched. On `list_item` that is the row the view's
`$select` projected, so a column the view did not ask for is **absent**, not
`null` — and CEL aborts the whole expression at key resolution (`No such key:
status`). The abort is fail-closed, so the button simply is not offered, which
looks exactly like the predicate having said no. Nothing logs it.

| what the predicate does with `record.x` | guard |
|---|---|
| compares it to a literal (`==`, `!=`) | `has(record.x) && record.x == 'draft'` — `has()` alone. CEL compares heterogeneously, so a projected-`null` column answers `false` rather than faulting |
| traverses, calls a method, orders or does arithmetic | `has(record.x) && record.x != null && record.x.size() > 0` — both halves |
| traverses into a nested block | guard the **leaf**: `has(record.x) && has(record.x.k) && record.x.k == true`. The leaf guard subsumes the parent `!= null` |
| tests truthiness (`!record.x`, bare `record.x`) | rewrite to the equality form — `has(record.x) && record.x != true` / `== true`. `!` and the logical operators need a bool and **fault** on `null` |

The last row is the one that bites: `visible: "!record.disabled"` faults on
every row whose `disabled` column is stored `NULL`, so the button vanishes from
exactly the records it was meant for.
</Callout>
...
# (has(record.disabled) && record.disabled != true) && features.oidcProvider != false
