...
`collapsed: true` implies `collapsible` — a section that starts closed always carries the disclosure
control that reopens it — and the example writes both keys so that control is explicit on every form surface.
...
|----------|-----------|--------|
| `@objectstack/react-renderer` | React + Material-UI | 🚧 In Progress |
| `@objectstack/vue-renderer` | Vue 3 + Vuetify | 📋 Planned |
| `@objectstack/flutter-renderer` | Flutter (iOS/Android) | 📋 Planned |
| `@objectstack/swift-renderer` | SwiftUI (iOS native) | 📋 Planned |
| `@objectstack/compose-renderer` | Jetpack Compose (Android) | 📋 Planned |
| `@objectstack/tauri-renderer` | Tauri + React (Desktop) | 📋 Planned |
...
- [View API](/docs/references/ui/view) - Complete FormView & ListView configuration reference (table, kanban, calendar)
