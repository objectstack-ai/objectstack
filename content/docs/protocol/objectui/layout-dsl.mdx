...
Virtualization for large datasets is configured on the **view**, not on a section
or a layout: set the boolean `virtualScroll` on a list-shaped view
(`ListViewSchema`). That is the only virtual-scrolling switch objectui reads —
see [Widget Contract → Performance](/docs/protocol/objectui/widget-contract#performance).
...
<Callout type="info">
  ...
  implemented**. For the keys a section really accepts, see the
