...
Three members left this table under ADR-0049 enforce-or-remove (#4286): `joins` and
`windowFunctions` were **removed** — tombstoned in `@objectstack/spec` 18, so a query
...
SQL-driver door, `SqlDriver.findWithWindowFunctions()`) — and `having` was **enforced**
(see §5). The inert members that remain declared (`cursor`, `distinct`, and the
experimental flags above) are tracked in the liveness ledger
(`packages/spec/liveness/query.json`) pending their #4286 dispositions.
</Callout>
