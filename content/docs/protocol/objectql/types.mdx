...
**Database mapping:**
- SQL driver: an exact-decimal column — `NUMERIC(65,30)` on PostgreSQL,
  `DECIMAL(65,30)` on MySQL, and `float` on SQLite, which applies neither
  precision nor scale. The pair comes from one per-field-type table in
  `packages/spec` (`NUMERIC_COLUMN_REPRESENTATION`) that every producer of DDL
  reads, and `65,30` is MySQL's documented `DECIMAL` maximum — the portable
  ceiling rather than a taste. A field's own `precision`/`scale` stay validation
  and display metadata: they do **not** size the column, so the DDL still never
  emits a `NUMERIC` sized from your declaration.
- ⚠️ New columns only. A column created before this landed keeps the `REAL` /
  `FLOAT` it already has — nothing retypes it, and no migration is planned.
- MongoDB: `Number`
...
**Database mapping:**
- SQL driver: an exact-decimal column (`NUMERIC(65,30)` / `DECIMAL(65,30)`, and
  `float` on SQLite) — one column, no companion currency column and no JSON blob.
  Money is the member the exactness is for: the `REAL` column this replaced was
  measured to take `1234567.89` and read it back `1234567.9`. New columns only —
  an existing `REAL` money column is left exactly as it is
- MongoDB: `Number`
...
**The day you read back does not depend on the app process's timezone.** The SQL
driver pins each dialect so a stored `DATE` arrives as the calendar-day string
itself, never as an instant it would then have to re-derive a day from:
PostgreSQL connections parse `date` (and `date[]`) as text, and MySQL
connections are pinned to UTC in both directions. So an app container running
`TZ=Asia/Shanghai` and one running `TZ=UTC` read the same row as the same day —
running the process at UTC is not a prerequisite for correct dates.
...
| `number` / `currency` / `percent` | `NUMERIC(65,30)` ‡ | `DECIMAL(65,30)` ‡ | `float` ‡ |
...
‡ The numeric family takes one physical representation, stated per field type
in `packages/spec` (`NUMERIC_COLUMN_REPRESENTATION`) and read by every producer
of DDL — `SqlDriver.createColumn` and both `os generate migration` formats — so
one declaration can no longer produce three different columns. `number` /
`currency` / `percent` / `slider` / `progress` / `summary` are the exact-decimal
members; `rating` is `INTEGER` on PostgreSQL and MySQL, which is where a
fractional star count is refused (SQLite refuses no fractional value and stores
it as a REAL). On SQLite knex compiles the exact-decimal column and the `float`
column it replaced to the identical `float` DDL, so that dialect keeps REAL
affinity and gains no exactness. **New columns only** — a column that already
exists keeps its `REAL` / `FLOAT`, and no migration is planned for it.
