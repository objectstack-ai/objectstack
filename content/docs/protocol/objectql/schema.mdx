...
| `scale` | `number` | `number`, `percent`, `rating`, `slider` | Decimal places, enforced on writes. Refused on `currency`: a currency amount's decimal places come from its currency's ISO 4217 minor unit. |
