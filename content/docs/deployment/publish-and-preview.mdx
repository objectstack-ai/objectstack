...
# no --env: installs into the environment `os environments switch` last selected
...
| `--env` | `OS_ENVIRONMENT_ID` | Environment to install the new version into. Defaults to the environment `os environments switch` recorded for this control plane in `~/.objectstack/cloud.json` |
