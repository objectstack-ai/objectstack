...
The table below has exactly two axes: its **rows** are rule **families**, its
**columns** are the four doors. A `✓` means that door runs that family — it does
not say what that door hands the family to judge. That input is a third thing,
orthogonal to both, and no cell here can carry it; it is written out in
[What each door hands the rule table](#what-each-door-hands-the-rule-table).
...
`object`, `✓ᵖ` `page`, `✓ˢ` `seed`, `✓ᵇ` `book`, `✓ᵖᵉ` `permission`,
`✓ᵖᵒ` `position`, `✓ᵃ` `app`** — the two `ᵖ`-initial pairs carry a second
letter because `page` already holds `ᵖ` on its own. Read the codes
**longest-first**: `ᵖᵒ` is `position`, never `page` followed by `object`. A cell
lists one superscript per declared type, in the order the rule declares them, and that
...
`object`, #4716 crossed the five remaining **gating** object rules —
functional completeness, managed API methods, autonumber formats, and both
validation-rule enforceability checks — onto `object`, and #19370 took the
vocabulary freeze across all five of its own write types at once), so read the
rule rather than assuming a save of some other type reaches storage unjudged.
...
crossed, because at that moment the door built no snapshot for two of the
collections it judges — `positions` and `apps`: one rule id has to sit on ONE
side of the wall, so it stayed behind whole rather than crossing for some of
the collections it judges and not others.

**It crossed in #19370, also whole**, and the split is what made that possible
one half at a time. `position` and `app` are runtime-creatable — Studio's app
designer is a shipped tenant capability — so the two types were mapped at the
door rather than withdrawn from authors, and the rule now declares the write
type of every collection it judges. A runtime write is refused when the
reserved word appears in an object, field, action or field-group identifier or
label, a permission-set name or label, a book's, a **position's**, or an
**app's**. The CLI verdict is unchanged: both security entries have run on all
three commands since the split.
...
missing. The freeze kept one id whole by staying behind and then crossing whole;
here four ids stopped agreeing, and one row cannot say two things. **When the next crossing lands on
...
the snapshot has carried `permissions` and `books` since #8309 and `datasets`
since #7529, so those three are no longer in this class, and `positions` /
`apps` left it by the other route in #19370: the only rule that reads them
resolves nothing across collections, so mapping the two write types was enough
and the snapshot did not have to grow);
...
Read that parenthetical at the scope it is written at. It is a claim about these
**rows**: no row is `✓` at one door and `—` at another in a direction that lets a
stack through. The same sentence about the **input** each door hands the table is
a *different* claim, it has been false three times, and the two read identically
in prose — so the input claim is stated
[separately below](#what-each-door-hands-the-rule-table) rather than folded in
here.
...
Everything above this heading is one axis: which rule **family** runs at which
**door**. This is the other one — the **input** each door hands those families to
judge. They are independent: a door can run every rule in the registry and still
certify a project clean, if what it hands them is an empty stack. A `✓` in the
matrix answers the first question only.

**The three CLI commands run the whole table twice on every run.**

1. **The union fold.** One stack, every package's collections seen together.
   Under the ADR-0130 D4 artifact shape a project's definitions may live entirely
   in `packages[]` with nothing at the top level, so the fold fills only the
   collections the top level does not carry, and returns a stack that already
   carries them unchanged. This is the only pass that can see a finding which
   **spans** packages. Until it was wired at all three doors (#17069, #17528),
   such a project was judged as if it declared nothing — `os validate` and
   `os lint` exited 0 on a stack `os build` refused.
...
this stack defines?"* is satisfied by the union for a package that carries
neither half; per package it is not. Because the artifact registers **per
package** (ADR-0130 D4/D5), the per-package answer is the one the runtime will
live with.

**That pass reached the three doors one at a time, and every gap was false-clean
in the author's favour.** `os build` has run it since #16611, `os validate` since
#18677 and `os lint` since #18778; until each of those landed, the door in
question ran the union fold and stopped. Symbol presence is not coverage here:
`os lint` imported the per-package seams for two cards before it ran this pass,
because it walks `packages[]` for a duplicate-name advisory of its own — and that
advisory is `os lint`'s own rubric, not this table.

**The fourth door is on neither pass, and the package narrowing it does have is a
third thing again.** A runtime write is one *item*, not a stack, so the publish
gate builds a per-write snapshot — the written item plus the live registry's
objects as resolution context — and runs the table twice over it: once on the
context alone, once with the item grafted in, attributing to the write only the
findings the item **added** (#4463). Its package dimension narrows that context to
the written item's **package closure** — the written package, its declared
dependencies, and unconditionally both platform/system objects and rows carrying
no package provenance (#9612); a provenance it cannot read narrows nothing at all.
That changes what a rule can **resolve**, never what it judges, and nothing at
this door walks `packages[]`. So "runs per package" names one thing at the three
CLI doors and another one here, and the `runtime publish` column says neither.
