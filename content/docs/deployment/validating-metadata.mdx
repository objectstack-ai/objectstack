...
### 9. Keys the schema never declared

`ObjectStackDefinitionSchema`, `ObjectSchema` and `FieldSchema` are deliberately
not strict, so a key none of them declares **parses clean and is dropped** on the
way to storage. Nothing fails; the setting simply is not there.
...
The same applies to the stack's own **top-level** keys, which is where the
silence is easiest to miss — the key looks like configuration that took effect:

```
stack.datasource: 'datasource' is not a declared stack key, so its value is
  dropped at load — did you mean 'datasources'?
stack.storage: 'storage' is not a declared stack key, so its value is dropped at
  load — not a stack key — the file-storage backend is a deployment concern, not
  an application declaration. Configure it with the OS_STORAGE_* environment
  variables, or per-deployment in Setup → Settings → Storage.
```

`storage` is the worked example: `os serve` used to read it on the one boot path
that skips `defineStack`, so the same key configured a backend in one place and
disappeared in every other — a stack asking for S3 could silently get local disk
(#4167). Storage is configured through [`OS_STORAGE_*`](/docs/deployment/environment-variables)
or the Settings UI, which is also where credentials belong: a stack definition is
committed to git and compiled into any published artifact.
...
| Undeclared stack/object/field keys (#3786, #4167) | ✓ | ✓ |
