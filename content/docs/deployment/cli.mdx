...
the environment `os environments switch` recorded **in `cloud.json`**, and that
...
| `--env <id>` | `OS_ENVIRONMENT_ID` | Environment to install the new version into. Defaults to the environment [`os environments switch`](#cloud-environments) recorded **for this control plane** in `~/.objectstack/cloud.json` |
