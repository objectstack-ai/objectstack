...
3. **RLS: OR within an object, AND with tenant-global.** Multiple row policies
   ...
   is derived at one site, and which anchors it reads depends on the
   deployment's [tenancy posture](/docs/deployment/tenancy-modes). The
   deployment's **configured administrator list** — an address declared in
   `OS_PLATFORM_OWNER_EMAIL`, matched against the caller's own stored and
   **email-verified** `sys_user` row — is sufficient under every posture. An
   **unscoped `admin_full_access` grant** row is sufficient only under
   `single`; under the walled postures (`group` / `isolated`) it is no longer
   an anchor — the set it names still grants what any held set grants, but it
   confers no `PLATFORM_ADMIN` standing, so there standing comes from
...
judged like deleting or renaming the row (ADR-0135 D5.2): it is refused while
