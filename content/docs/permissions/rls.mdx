...
| `check` | `string` | Predicate the new row must satisfy **after an insert or update**. Omit it and `using` stands in, unless another applicable policy for the same operation declares a `check` — see [the fail-closed contract](#the-fail-closed-contract) |
...
4. `check` is omitted → `using` stands in as the write check. The choice is
   ...
   decide, and a USING-only sibling's `using` is not part of the write check.
