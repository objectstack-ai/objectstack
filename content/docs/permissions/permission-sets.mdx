...
standing is derived at a single site from **two anchors**. Which of them is
sufficient depends on the deployment's
[tenancy posture](/docs/deployment/tenancy-modes):
...
- **Configuration — `OS_PLATFORM_OWNER_EMAIL`.** The deployment declares its
  ...
  This anchor is read under every posture.
- **An unscoped `admin_full_access` grant row** — a `sys_user_permission_set`
  row with `organization_id = NULL`, the original anchor. It confers standing
  under the `single` posture only. Under the walled postures (`group` /
  `isolated`) the row is no longer an anchor: the set it names still grants
  what any held set grants, but it confers no `PLATFORM_ADMIN` standing, and
  nothing is logged about it per request. If a walled deployment holding such a
  row ends up with no usable configured administrator, the boot log says it has
  zero platform administrators, names the account that holds the row, and points
  at `OS_PLATFORM_OWNER_EMAIL` as the way to restore that account's standing.
