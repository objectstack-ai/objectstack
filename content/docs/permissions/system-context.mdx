...
cannot set it (`packages/rest/src/rest-server.ts:1236`, `:1265`), and neither
...
| 17 | REST anonymous-deny seam satisfied | rest | Get: `enforceAuth` passes with no `userId`. Not reachable from the wire — `isSystem` is never set on an inbound request | `rest-server.ts:1268` |
...
| 50 | `manage_metadata` bypassed on metadata writes | runtime, rest | Get: schema writes without the capability | `domains/meta.ts:471`, `:874`, `rest-server.ts:4272`, `:5635`, `:5867`, `:6230`, `:6423` |
...
| "A client can request it" | **No.** Never settable from inbound HTTP or from an action body | `rest-server.ts:1236`, `:1265`; `domains/actions.ts:404` |
