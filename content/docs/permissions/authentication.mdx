...
| `403` | `ORGANIZATION_MEMBERSHIP_LIMIT_REACHED` | The organization is at its membership limit (`OS_ORG_MEMBERSHIP_LIMIT`; unset means no limit). The platform always passes an explicit limit to better-auth, so the vendor's own 100-member fallback never applies. |
