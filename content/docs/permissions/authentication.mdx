...
its own — measured on the then-installed 1.7.1, where `addMember` builds its
endpoint with no path argument while every sibling in the same module
(`/organization/remove-member`, `/organization/list-members`,
`/organization/leave`, …) passes one. So the vendor's documentation does not
list this URL, and an audit that enumerates better-auth's mounted surface will
not find it there either. ObjectStack mounts it ahead of the
