...
Better-Auth is a direct runtime dependency of the plugin (`better-auth` in
`packages/plugins/plugin-auth/package.json` — there is no `peerDependencies`
block), so it is installed with the plugin; you do not add it yourself.
