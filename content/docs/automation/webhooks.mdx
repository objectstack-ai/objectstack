...
refused (`VALIDATION_ERROR` / `400`). Register a `CryptoProvider` so the sweep
can convert the row, or move the values into their encrypted columns. Never type a token into
