...
|-------------------|-----------|------------------------------------------------------------------------------------|
| `id`              | text      | Primary key.                                                                       |
| `name`            | text      | snake_case name, unique **per organization** ([#8554](https://github.com/objectstack-ai/objectstack/issues/8554)) — referenced in logs and audit. |
...
deliberately left intact — is **not** delivered: the subscription is parked and
each event is recorded in `sys_http_delivery` as a dead row carrying a dated
refusal, and a write that puts `secret` or `headers` into `definition_json` is
refused (`VALIDATION_ERROR` / `400`). Both remedies need a registered
`CryptoProvider`. With one registered, either restart so the sweep converts the
row, or move the values into their encrypted columns yourself. Never type a token into
...
|--------|-----------------------------|-------------------|--------------------------------------------------|
| POST   | `/api/v1/webhooks/redeliver` | `{ deliveryId }`  | Re-queue a delivery that was **attempted** and failed. A row that was never attempted cannot be redelivered. |
...
`RESOURCE_NOT_FOUND`, `409` `DELIVERY_NOT_ELIGIBLE`, or `409`
`DELIVERY_NEVER_SENT` (ADR-0112). The last one is the refusal behind the table
row above: a row with `attempts = 0` is a durable record of a delivery that
could never be prepared — its webhook's signing secret could not be decrypted —
so it carries **no HMAC signature**, and re-queuing it would not replay
anything, it would send the payload for the first time and send it unsigned.
Fix the signing configuration instead; the subscription re-arms on its own and
subsequent events are delivered signed. There are no
