...
engine logs a warning and falls back to considering every out-edge — which is
how the mismatch above shipped silently. `os validate` reports the same shape as
`flow-branch-label-unmatched` at build time, along with
