...
ledger's `value` role and `AssignmentConfigSchema`, the half ruled in
...
| `failed` | Node executions that **failed and were contained** — caught by a `try_catch` or routed down a `fault` edge — so the run went on; the sum of `nodes[].failures`. Absent on a run that did not track it, which is not zero |
