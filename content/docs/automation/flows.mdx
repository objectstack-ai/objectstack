...
startup summary, in `getTriggerBindingAudit()` and in Studio as **disabled by
deployment policy** — a distinct reason, never "binding failed". Nothing about
the flow needs fixing; the deployment has not asked for it. `os doctor` prints
the effective value.
