...
any production path: the only `new LayeredRepository(...)` call sites are its own
tests.
</Callout>
...
> propagation is instead a **cluster** concern: install `@objectstack/service-cluster`
> and its `MetadataClusterBridgePlugin` calls `MetadataManager.attachClusterPubSub()`
> at `kernel:ready`, fanning every local watch event out over the `metadata.changed`
> channel (loopback-suppressed by `originNode`) so peer nodes invalidate their
> caches. Note this replays the *invalidation event*, not the overlay row — peers
> re-read from the shared database. Without the cluster service registered, the
> bridge logs and skips, and each replica only sees its own writes.
