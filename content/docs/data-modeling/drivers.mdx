...
| :--- | :--- |
| A **declared datasource** — `{ driver: 'memory' }` in a stack/app config | **Ephemeral.** Nothing is written to disk unless the declaration sets `config.persistence`. |
...
state in the working directory. Note that two `'auto'` drivers in one process
share that one default path.
