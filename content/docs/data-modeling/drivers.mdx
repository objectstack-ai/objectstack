...
[`mingo`](https://github.com/kofrasa/mingo)). By default data is lost when the
process exits. It is the **last-resort fallback** in dev mode: `objectstack dev`
prefers native SQLite (`better-sqlite3`), falls back to the pure-JS WASM SQLite
driver if the native binary is unavailable, and only drops to the in-memory
driver if WASM also fails to load. Set `OS_DATABASE_DRIVER=memory` to select it
explicitly.
