...
manifest fields and the metadata it owns. Each definition is written **once**,
in the body of the package that owns it, beside the artifact's own `manifest`.
`os dev` boots the same shape straight from source, and `GET /api/v1/packages`
on a running instance lists every package in the artifact.
