...
<Callout type="warn">
**The config file is a module, and the whole module is the stack.** What
`defineStack()` returns is the *base*; every **named export** of
`objectstack.config.ts` is then merged onto it as a **top-level stack key**,
under the export's own name. That is how the `onEnable` and `functions` hooks
are authored — and it is why a helper exported beside the stack is not read as a
helper:

```typescript title="objectstack.config.ts"
export default defineStack({ /* … */ });

// ⛔ Read as a top-level stack key called `collectPackageDirs`.
//    `os build` refuses it: Unrecognized key(s) on this stack definition.
export const collectPackageDirs = () => [ /* … */ ];
```

A named export is legal **only when its name is a key the stack schema
declares**. Keep helpers in a sibling module — `objectstack.composition.ts`,
say — and import them here. Two further shapes worth recognising, neither of
them a pattern to rely on:
