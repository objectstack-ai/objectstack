...
| **actor** | `string \| null` | ✅ | Who wrote this. `null` = system-initiated (boot sync, migration, scheduled job) — never a sentinel string (#4556): consumers that resolve this against `sys_user` must be able to tell "nobody" from "a user id". |
