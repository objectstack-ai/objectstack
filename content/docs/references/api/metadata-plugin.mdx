...
```typescript
import { MetadataBulkRegisterRequestSchema, MetadataEventSchema } from '@objectstack/spec/api';
import type { MetadataEvent } from '@objectstack/spec/api';

// Validate data
const result = MetadataBulkRegisterRequestSchema.parse(data);
```
