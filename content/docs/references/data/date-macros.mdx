...
form — canonical UTC text on SQLite, a native `timestamptz` on Postgres,

a `DATETIME(3)` literal on MySQL, and `YYYY-MM-DD` text for a calendar

day on every dialect — is the driver's job; see
