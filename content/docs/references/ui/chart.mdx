...
### Nested Shape: `ChartAggregate.groupBy`
...
| **xAxis** | `{ field: string; title?: string \| Record<string, string>; format?: string; min?: number; … }` | optional | X-Axis configuration. Structure, not appearance: on a dataset-bound dashboard widget the dataset decides this and the key is refused by name (ADR-0021) — select `dimensions` / `values` on the widget instead. It stays authorable on an inline-data react `<ObjectChart>`. |
| **yAxis** | `{ field: string; title?: string \| Record<string, string>; format?: string; min?: number; … }[]` | optional | Y-Axis configuration (support dual axis). Structure, not appearance: on a dataset-bound dashboard widget the dataset decides this and the key is refused by name (ADR-0021) — select `dimensions` / `values` on the widget instead. It stays authorable on an inline-data react `<ObjectChart>`. |
...
### Nested Shape: `ChartConfig.xAxis`
...
### Nested Shape: `ChartConfig.yAxis[number]`
...
### Nested Shape: `ChartConfig.series[number]`
...
### Nested Shape: `ChartConfig.annotations[number]`
...
### Nested Shape: `ChartConfig.interaction`
