...
| **measures** | `{ name: string; label?: string \| Record<string, string>; aggregate?: Enum<'count' \| 'sum' \| 'avg' \| 'min' \| 'max' \| 'count_distinct' \| 'array_agg' \| 'string_agg'>; field?: string; … }[]` | ✅ | Aggregatable values |
