...
```typescript
import { NotificationSchema, NotificationActionSchema, NotificationConfigSchema, NotificationPositionSchema, NotificationSeveritySchema, NotificationTypeSchema } from '@objectstack/spec/ui';
...
| **actions** | `{ label: string; action: string; variant: Enum<'primary' \| 'secondary' \| 'link'> }[]` | optional | Action buttons |
