...
### Nested Shape: `BulkActionDef.params[number]`
...
| **type** | `Enum<'text' \| 'textarea' \| 'email' \| 'url' \| 'phone' \| 'password' \| 'secret' \| …>` | ✅ | Field widget to render, from the standard field-type vocabulary (text/number/select/lookup/date/…). |
...
### Nested Shape: `BulkActionParam.options[number]`

| Property | Type | Required | Description |
| :--- | :--- | :--- | :--- |
| **label** | `string` | ✅ | Option label (plain string — not i18n-resolved on this path). |
