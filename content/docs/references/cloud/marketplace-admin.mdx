...
```typescript
import { CuratedCollectionSchema, FeaturedListingSchema, MarketplaceHealthMetricsSchema, PolicyActionSchema, PolicyViolationTypeSchema, RejectionReasonSchema, ReviewCriterionSchema, ReviewDecisionSchema, SubmissionReviewSchema, TrendingListingSchema } from '@objectstack/spec/cloud';
...
// Validate data
const result = CuratedCollectionSchema.parse(data);
```
...
| **criteria** | `{ id: string; category: Enum<'security' \| 'performance' \| 'quality' \| 'ux' \| 'documentation' \| 'policy' \| 'compatibility'>; description: string; required: boolean; … }[]` | optional | Review checklist results |
...
### Nested Shape: `SubmissionReview.criteria[number]`
