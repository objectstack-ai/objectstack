...
| **source** | `{ type: Enum<'database' \| 'api' \| 'file' \| 'stream' \| 'object' \| 'warehouse' \| 'storage' \| 'spreadsheet'>; connector?: string; config: Record<string, any>; incremental?: object }` | ✅ | Data source |
...
| **transformations** | `{ name?: string; type: Enum<'map' \| 'filter' \| 'aggregate' \| 'join' \| 'script' \| 'lookup' \| 'split' \| … +3 more>; config: Record<string, any>; continueOnError?: boolean }[]` | optional | Transformation pipeline |
