...
with simple `auth` — or by `[automation/etl.zod.ts](/docs/references/automation/etl)` for transformation
...
| **fieldMappings** | `{ source: string; target: string; transform?: any; defaultValue?: any; … }[]` | optional | Field mapping rules |
