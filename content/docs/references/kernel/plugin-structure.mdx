...
```typescript
import { OpsDomainModuleSchema, OpsFilePathSchema, OpsPluginStructureSchema } from '@objectstack/spec/kernel';
import type { OpsDomainModule, OpsFilePath, OpsPluginStructure } from '@objectstack/spec/kernel';

// Validate data
const result = OpsDomainModuleSchema.parse(data);
```
