...
ObjectStack provides **nine domain-specific skills**, one per protocol domain — Platform, Data, Query, UI, Automation, AI, API, i18n, and Formula — plus **Upgrade**, a `process` skill that teaches no schema: it carries a metadata project across a protocol major, running the deterministic conversion chain first and then working the semantic residue the chain cannot express.
