...
<Callout type="info">
**Architecture Overview:** For a high-level view of how protocol layers connect, see the [Protocol Relationship Diagram](/docs/protocol/diagram).
</Callout>
...
flowchart LR
    ...
    subgraph "Runtime"
        ...
        H -->|"serve"| I["GET /api/v1/meta/object"]
...
| Typed Client | Frontend apps | `client.data.create('task', { title: '...' })` |
...
flowchart TD
    subgraph "Insert Operation"
        C4[beforeInsert] --> C5["DB INSERT"]
        C5 --> C6[afterInsert]
    end

    subgraph "Update Operation"
        U4[beforeUpdate] --> U5["DB UPDATE"]
        U5 --> U6[afterUpdate]
    end
...
|:---|:---|:---|:---|
| `beforeInsert/Update/Delete` | Pre-persist | ✅ Yes (mutate `input`) | ✅ Yes (throw with `onError: 'abort'`) |
| `afterInsert/Update/Delete` | Post-persist | ✅ Yes (mutate `result`) | ❌ No (record is saved) |

<Callout type="tip">
**Side Effects:** Place side effects (emails, notifications, external API calls) in `after*` hooks. Set `async: true` so they run fire-and-forget without blocking the operation, and use `onError: 'log'` so a failing side effect doesn't roll back the write.
</Callout>
