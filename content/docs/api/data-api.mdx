...
| `filter` | query | Filter expression (JSON). `filters` also accepted for backward compatibility. Malformed JSON is rejected with `400 INVALID_REQUEST` — never ignored. |
...
`400 INVALID_REQUEST` rather than dropped, because a dropped filter would
