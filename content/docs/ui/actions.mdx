...
A failure carries a **real HTTP status** — 403 denied, 404 no such action, 400
for a business rejection or a param-contract violation, 500 for an unexpected
fault — with `{ "success": false, "error": { "message", "code" } }`. Read the
status; a 200 is the only answer that means the handler ran.
