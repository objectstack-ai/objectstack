...
<Callout type="warn">
  **`ctx.record` is read-only — persist through `ctx.api`.**
  ...
  `os compile` warn on a literal `ctx.record.<field> = …`, and the sandbox logs
  the discarded fields at invocation time — including the computed keys and
  aliased references static checks cannot see.
</Callout>
