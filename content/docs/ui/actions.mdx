...
`@objectstack/spec/ui`), so a bare filter is a **compile error** at the call
site — `{ status: 'completed' }` has nowhere to land, and so does a filter held
in a `FilterCondition` variable. A hand-written test double must honour the
envelope too.
