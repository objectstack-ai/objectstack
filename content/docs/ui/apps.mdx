...
- **Audience** — the entry has no gate of its own; it inherits the docs audience
  ...
  read is not shown. The page content itself is gated on the server by the
  `/meta/doc` and `/meta/book/:name/tree` reads. `visible` and
  `requiredPermissions` still apply and can only narrow further.
