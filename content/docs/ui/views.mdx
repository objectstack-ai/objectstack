...
fields: [
  {
    ...
    span: 'full',                // Whole row at the top breakpoint; below it, same as omitting
...
| `span` | `'auto' \| 'full'` | Relative width — `'full'` compiles to a span of the section's declared column count, gated at the top breakpoint only: the whole row there, identical to omitting the key at narrower widths |
