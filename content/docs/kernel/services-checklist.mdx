...
`setDefaultLocale` and `setSupportedLocales` but **not** `setFallbackLocale`: `os serve`
constructs it with `fallbackLocale || defaultLocale || 'en'` already collapsed from the
stack config, so the probe skips step 2 and it keeps the value it was built with —
omitting a setter is per-value, not per-provider. Each setter is also guarded on the app
