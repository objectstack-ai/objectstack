...
2. Narrow what `getLocales()` REPORTS to the app's declared `supportedLocales`, via
   `i18nService.setSupportedLocales()` (#7679)
3. Set the declared `i18n.fallbackLocale` — the locale `t()` consults after the requested
...
keeps its own behaviour rather than breaking; `FileI18nAdapter` is constructed with its
default and fallback locales and implements no setter for either. Each is also guarded on
the app having DECLARED the value, because several AppPlugins can share one kernel and an
app that declares no `i18n` block must not clear what another declared.
