...
- ❌ Plugin Required — the remaining 36 protocol methods (`analytics` 2, `automation` 1,
  `packages` 6, `views` 5, `permissions` 3, `workflow` 3, `realtime` 6, `notification` 7,
  `i18n` 3), delivered by whatever fills the slot
</Callout>
...
| 10 | **ai** | `optional` | — | ❌ Nothing ships in this repo | `service-ai` (Cloud/EE) |
...
<Callout type="info">
The Provider column is not prose — it is `CORE_SERVICE_PROVIDER` in
`@objectstack/spec/system`, the single table discovery reads when it tells a
caller what to install for an empty slot. `null` there (`ai`, `search`,
`workflow`) means **nothing ships**, and discovery says so rather than naming a
plausible package: `No implementation ships for the '<slot>' slot — register a
service under it to enable`. `ui` is the one slot with a hand-written remedy,
because the package named does not fill the slot: `Served by the protocol
service — register MetadataPlugin (@objectstack/metadata-protocol) to enable`.
`scripts/check-service-providers.mjs` fails CI if a name in that table is not a
