...
|:---|:---|:---|
| `DashboardWidget` | `object`, `categoryField`, `categoryGranularity`, `valueField`, `aggregate`, `aggregation`, `rowField`, `columnField`, `xAxisField`, `yAxisFields`, `measures` (and the `WidgetMeasure` type) | `dataset` + `values` |
...
`Report.columns` is repurposed rather than simply deleted: on both the report
and a joined block the key now names the dataset **dimensions across** a
`matrix` report, not raw report columns.
...
Eight chart types are removed, so the taxonomy no longer advertises variants
the renderer doesn't draw. Five of them (`bi-polar-bar`, `step-line`, `spline`,
`pyramid`, `bubble`) only ever rendered as their base chart. The other three
are dropped for a different reason: stacking is not a chart family but a
property of the series — `ChartSeries.stack`, where series sharing a group id
stack and otherwise group — so one `bar` / `area` family plus a series-level
stack group expresses all three. Update any widget or series using a removed
type:

| Removed type | Use instead |
|:---|:---|
| `grouped-bar`, `stacked-bar`, `bi-polar-bar` | `bar` (to stack, give the series a shared `stack` group id) |
| `stacked-area` | `area` (with a shared `series[].stack` group id) |
...
> Note: a later release added one family the taxonomy had never named —
> `combo`, for mixed bar/line/area series on shared dual axes, which the
> renderer does draw distinctly (configured with `ChartSeries.type` +
> `ChartSeries.yAxis`).
...
> Note: this has since shipped — `os package publish --visibility` accepts
> `private` / `org` / `marketplace` and defaults to `org` ("auto-visible /
> installable across your organization's environments"). See the
> [Publish & preview guide](/docs/deployment/publish-and-preview).
