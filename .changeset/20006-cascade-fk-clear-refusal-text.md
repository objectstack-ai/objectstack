...
- **Unchanged:** a rule that reads a key its own record does not hold keeps today's text byte for byte, whichever key the fault reports. That covers a key read from `record` directly or through a field that is not a reference, and one read from `previous` directly or through any field, that the record or its previous row does not hold. A declared column always reads, as `null` when empty, so it never counts. A rule that reads through no reference keeps today's text too, and so does every write that is not a delete's reference cleanup.
