...
The durable write happens on **both** arms, not just the disable. `POST /packages` is a create that an already-installed id reaches through `overwrite`, and `DELETE /packages/:id` does not clear this record either, so an install could answer `201` with `enabled: true` while the state file still listed the id as disabled — and `SchemaRegistry.installPackage` reads that file at boot, re-installing the package DISABLED one restart later with nothing red in between. The mirror of that is why the write reads the ROW rather than the request: `installPackage` lands an id seeded into `initialDisabledPackageIds` at boot disabled whatever the request says, so an install with `enableOnInstall` absent — the SDK's default call — answered `enabled: false` and then cleared that operator's disable off disk, bringing the package back ENABLED one restart later. A flag-absent install no longer re-enables a package disabled before a restart, and it no longer clears the record either. Every install now persists the state it returned.
