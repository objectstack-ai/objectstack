...
(`liveness-live-elsewhere-property`, exported beside `liveness-dead-property` /
`liveness-experimental-property` / `liveness-planned-property`, and advisory-only
like them), its own message (`is enforced in a sibling repo, not here`) and its
own default hint, which keeps the property and points at the ledger row's
`evidence` for the enforcer. It deliberately does **not** reuse the `dead`
branch: that is the #11384 lesson, which is that verdicts imply OPPOSITE author
actions, and "Remove it" is the single most damaging sentence available about a
key whose enforcement is real and remote — deleting it tears out a live gate's
input. The sentinel throw stays for genuinely unknown statuses, with its
enumeration of the known ones updated.
