...
<!-- adr-0087: not-required (runtime-interface-only packages/services/service-messaging/src/http-outbox.ts#IHttpOutbox, packages/services/service-messaging/src/http-outbox.ts#RedeliverOptions, packages/services/service-messaging/src/messaging-service.ts#MessagingService) The changed members are `IHttpOutbox.redeliver` and `MessagingService.redeliverHttp`, on the symbols named above (`RedeliverOptions` is the new argument type). All three are TypeScript declarations in a service package with no `packages/spec` schema behind them: no metadata author writes a `redeliver` key, there is no authorable spelling and no `retiredKey()` tombstone, so `os migrate meta` has no stack source to rewrite. The change is a required second argument on two in-process methods — a compile error at every call site, which is the notification channel, not a silent runtime gap. -->
