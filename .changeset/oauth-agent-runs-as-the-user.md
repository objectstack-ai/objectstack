...
Purely additive on every published surface: two new optional members, one new exported interface (`DelegationNarrowing`), and one new evaluator method. No existing member changed shape, and the only behavioural change is on the delegated path with a ceiling that declares no depth.
