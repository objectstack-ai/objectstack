...
`filter-preset-comparand` now judges a list page's `interfaceConfig.filterBy` and a lookup field's `lookupFilters`, the two consumed filter carriers the shared filter walk never entered (#19791).
...
- **Unchanged:** `filter-empty-combinator` / `filter-empty-node` judge Mongo-shape nodes only, and both carriers are arrays. `flow-filter-token-unknown` walks `flows`, and no flow schema declares either key.
