...
`find()`'s contract says nothing about `groupBy` / `aggregations`, and the drivers disagree about them. `driver-sql` and `driver-rest` ignore both and return rows — which is the only reason this path ever worked. `driver-memory` **honours** them (`find()` → `performAggregation`), and so do the `aggregate(AST)` faces behind `driver-mongodb` / `driver-turso`. Against a driver of the second kind the one seam answered two different wrong things:
