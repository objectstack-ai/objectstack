...
The same edit reconciles the `resolvePermissionSetsForContext` docblock, which until now assigned the `objects` merge to the caller and named its rule ("most-permissive"). That sentence would otherwise ship in the same `.d.ts` as this member's "ask instead of folding", 46 lines apart, telling one consumer to fold `objects` and the next never to. The `objects` axis is now documented as preferring this method, with the caller-side fold kept as the stated FALLBACK for a service that predates it; the other three axes (`fields`, `systemPermissions`, `tabPermissions`) are untouched.
