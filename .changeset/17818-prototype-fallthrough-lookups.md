...
Measured on Node v22.22.2 against the built artifact, before and after:
...
⚠️ **Why `minor` and not `patch`.** These are published exports whose observable
answer changes for a class of input, so the level follows the widening rule for
a declared contract change rather than the bug-fix default. Nothing in the
declared vocabulary moves: every canonical operator, every `EnvironmentType`
bucket, both operator shorthands and every manifest collection spelling answers
byte-identically to before, and the only inputs whose answer changes are the
four prototype-member spellings above, which no signature ever admitted.
