...
- 分诊席跑普通直连会话,达档职责改派显式传档位的子代理;自排定时不超过 55 分钟。
...
- 六个 pm 状态标签与 `needs-user-decision` 互斥且转换恒一笔 replace,队列卡逾三天欠一次转换。
...
- 交接按收尾清单逐步走完并清点自设定时器;归档本席派出的会话是不可移交的义务。
## 分诊座位职责
...
- 认领是原子对:同一次标签写入完成认领与状态对调,随后留以 `Claim:` 开头的认领评论。
- 认领评论写会话、分支、域、文件面、档位与串行约束,上墙后重读全线程。
...
- 每轮打中文轮次报告,含判决表、升级项、代裁清单、等人合项与受管合并审计清单。
- 五个健康指标:可派发库存、决策箱、finding 数与年龄、裸卡数与年龄、发版阻塞。
