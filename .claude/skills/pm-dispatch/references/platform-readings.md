...
## 断粮检测、预警节流与跨墙恢复细则(用量墙)

- **两级信号(维护者 2026-08-13 裁定)**:权威的**预警**信号是宿主注入的
  `rate_limit_info`(`status: "allowed_warning"` —— 请求仍被放行,墙未到)⇒ 当轮起
  进保守派发姿态(下条);权威的**墙**信号仍是失败本身:撞墙时 API 调用失败、宿主报
  「limit reached, resets at HH:MM」—— 重置时刻通常在撞墙那一刻可得,只是事前查不
  到;把它记下来。边界两侧都别越:预警不是墙 —— 轮次照跑,只换派发姿态,把预警当墙
  停摆等于白弃本周剩余额度;反向同理 —— 没见过预警不等于离墙还远,判墙只认失败本身。
- **保守派发姿态(`allowed_warning` + `seven_day`)**:批次上限 1;机械型 S/M 卡降
  到 `sonnet` 地板;冻结 L/XL 与非紧急工作;只保 `priority:p0`、release-board 项与
  在飞收尾;**fable-mandatory 卡(维护者裁定绑定 `claude-fable-5` 的)只延期,⛔ 永
  不降级**;每轮轮次报告带一行周窗口读数(单容器读数 + 已知重置时刻 —— 舰队级只有
  人能聚合,这行就是现存最便宜的聚合点)。姿态跟信号走:`allowed_warning` 不再出现
  或窗口已重置 ⇒ 回常规派发。
- **实测 payload 形状(2026-08-12,单一来源 —— dev 转述;下次出现照原文入表)**:
  `rate_limit_info: {rateLimitType: "seven_day", status: "allowed_warning"}`
  (camelCase),重置 ≈ 5 天后。`rateLimitType` 是枚举 ⇒ 平台区分墙型;工作假设
  (⛔ 从下一个撞墙 payload 证实,不猜):5 小时滚动窗与 7 天窗**并存**,谁近谁绑,
  `ccusage blocks` 仍报 5 小时计费块。原始 payload 带不带阈值/剩余量字段未知 —— 下
  次捕获时记全。
- **检测**:`npx ccusage blocks` 在容器内可用(读本地会话记录),两个有用读数 ——
  当前 5 小时窗口的边界/剩余时间,与燃烧率(预警);`npx ccusage weekly` 给按周聚
  合,同一套本地记录。三个盲区连着读:单容器视野(云卡在自己容器里烧同一账号的额
  度,这里看不见 —— 周上限是账号级,单容器的周读数 ≈ 本会话自己,这个盲区在周墙下
  比在 5 小时墙下更糟);估的是成本不是套餐余量(没有任何面向 agent 的接口暴露账号
  级剩余额度);窗口起点是本地推断的近似值。
- **定时器选型**:拿到重置时刻 ⇒ 一发定点(reset + 缓冲)优先,一枪精确胜过逐小时轮
  询;`create_trigger` 的 `run_once_at` 支持多天后的定点,seven_day 墙就用这一发,
  ⛔ 绝不挂跨多天窗口的每小时 cron —— ~5 天 ≈ 120 发空 fire,外加一个要记得删的孤
  儿 cron。5 小时级没有重置时刻 ⇒ 挂每小时 cron Routine(`create_trigger`,cron
  型),⛔ 不用 send_later 链 —— send_later 是一次性触发器,fire 后自灭(平台文档:
  run-once,fire 后自禁用),投进死窗口的那一发是否会被平台重试**未实测且文档未承
  诺**,按保守设计,一次性链条可能在断粮窗口内烧掉唯一一发而断链,恰好断在它存在的
  意义上;cron 每小时重发,第一枪成功的 fire 跑恢复,然后**删除 cron**(自清理是纪
  律的一部分 —— 幸存 cron 是孤儿定时器)。fired 文本照定时器写法纪律:幂等开头、只
  写判据。
