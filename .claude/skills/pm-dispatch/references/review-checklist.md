...
- 已发布 = 各包 `files[]` 实际发运的内容;spec 发运 `src/**/*.zod.ts`,假 TSDoc/`.describe()` 即缺陷。
- `.js.map` 的 `sourcesContent` 不算已发布:构建关闭其内嵌源码,关闭前也不计。
- 本轮令其变假或触碰的已发布缺陷必修;只注意到、未触碰的立卡不修,记明已发布面。
...
- 报告清掉了它被要求停下的停条件 ⇒ 亲核证据再 ACCEPT,⛔ 不接受结论散文。
