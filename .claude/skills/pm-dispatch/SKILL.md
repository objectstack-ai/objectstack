...
**座位 Routine 三原则**(细则见 `references/seat-post-protocol.md`):① 维护者在 Routines UI 创建、UI 钉
模型;② 手动 fire 烟测,判据取**GitHub 上的产出**;③ 每 fire 一轮:读座位贴 → 从 labels 重建状
态 → 跑一轮 → 结束,不继承上下文。**开轮互斥读法**(宁少跑,不让两个同座位会话并行写
