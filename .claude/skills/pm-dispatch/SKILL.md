...
| `tooling` | 修复落在门禁/脚本/workflow/技能/席位协议/PM 工具面而非产品包;分诊首触打,与 `domain:*` 同笔 |
...
- `tooling` 四具名读者:候选查询排除、首触即关、舰队一张在飞、普查半态行。
...
- 定级按 `docs/NORTH-STAR.md`「优先级」第 1、2、3 条;正文首行 `Path: P<n> | <清单区> | none`。
- `Path: none` ⇒ 首触即关 not_planned,写明理由与上行两个重开条件;⛔ 不定 p3、不 hold。
- 产品仓 P0/P1 开着时,无解锁对象的 p2/p3 `tooling` 卡同样关,⛔ 不排队、不 hold。
