...
会话并行写标签):fire 开始、首个写动作之前读两个读数 —— 最近收班简报未满一个轮次,或最新「开
轮」标记晚于收班简报且自身未满一个轮次(轮中会话按设计还没发收班贴,开轮标记是它唯一可见的痕
迹)⇒ 自退;两读数皆清,先在座位贴留一行开轮标记(session ID + fire 时刻)再跑轮。跨 fire 长流
程的前提:接力/停放的每一项都落成 GitHub 上可读的文本,⛔ 不留在会话记忆里。
