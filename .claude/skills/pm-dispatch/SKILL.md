...
加**自退、永不清座位;两读判「非本 session」把 subagent dev 的同 ID 认领读作本会话在飞、
不自退,是特意保留的安全方向 —— 同会话 dev 本非互斥对象,甄别靠分支(见「认领」)。
四读数皆清,先在座位贴留一行开轮标记(session ID + fire 时刻)再跑轮。
...
- **写后回读按风险定向;标签写恒为硬步骤 read-modify-write + 对比式回读**(维护者 2026-08-18
  ...
  目标增删) —— 读与写之间落地的并发标签会被你的整组 PUT 静默剥掉,常剥你没打算碰的那
