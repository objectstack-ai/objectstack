...
| `mode:subagent` \| `mode:cloud` | 派发后端 | 按卡分流:M+ ⇒ `cloud`,S 级机械 ⇒ `subagent`(维护者 2026-08-10 裁定) |
...
| `pm:on-hold` | 决定**已做**且答案是「不是现在」—— 不派发也不催;hold 评论带**日期、理由、重启条件** |
| `pm:blocked` + 正文行 `Blocked-by: #N` | 等上游 —— 选择期跳过,#N 关闭时由解锁扫描放回 |
| `finding` | 观察类记录 —— 持有态,待发现分诊轮定级,不占队列不进收件箱 |
...
- **状态变更不过夜**:标签挂了评论没跟上、assignee 设了没认领评论、结论只在 chat ——都是半状态
  ,结束会话(含限流悬挂)前补齐成对或回滚半边(机械辅助
  :`node scripts/pm/check-half-states.mjs`,report-only 巡查)。**代执行他人指令的关闭/作废,评
  论带出处三件(谁的指令、原话、在哪说的)** —— 无出处的关闭与误操作在证据上不可区分,会被兄弟
  席当误扫重开;同理适用于摘标签、回收认领等不可反推理由的动作。
- **写后回读**:每次写正文、评论、标签,发出后读回校验 —— sanitizer 会静默吞内容, label bot 的
  整组 PUT 会冲掉刚打的手工标签;「API 返回 200」≠「落地内容正确」。

**一次性建标签**:`bash scripts/pm/ensure-pm-labels.sh`(幂等)。⛔ 退役的`domain:engine` /
`domain:ui` 不在脚本里也不要加回去 —— 重建退役标签就是把无主车道放回自动补全;两者存量已清零
但**标签对象仍在**,删除对象是 PM 的一步单独动作,删掉前别当它们不存在。
...
**跨座位转移**:工作跨座位线,PM 永不跨 —— 落到对方队列(目标仓立单带 `pm:queue` +出处行),依赖
走 `Blocked-by:`;后续杂事由消费侧座位立;凡触 `packages/spec` 一律转`domain:spec` 座位(唯一所
有者),不论谁需要它。
...
**跨域例外路径(唯一的越界通道)。** 真拆不动的跨域单 PR 由**分诊座位指定**一个车道PM 认领;认
领评论**申报完整文件面**;只在这条路径上跑**定向在飞检查**(范围 = 该单文件面触及的那几个域的
在飞单,读各自认领评论的文件面申报,要求不相交,相交即让行)。日常同域批次选择不跑全局在飞检
查 —— 同域独立性由本域批次选择保证,跨域相交只可能从例外路径进来。
...
多写手共编一个 body 机制上防不住互吞(维护者 2026-08-06 拍板):**贴 = 座位**(标签`pm:seat`),正
文固定六段现值(**范围 | 当前 PM | 继承台账 | 热文件串行队 | 常设承诺
| 说明**,没有一段随时间增长),**只由在任座位 PM 编辑**,正文为权威 —— 标题
(`[PM seat] <座位名> — <状态>`)与 assignee 是它的派生视图,三者同笔更新。接管/移交 = 改正
文 + 一条审计评论,**评论只作审计不承载状态**;接管压缩是标准步骤(按六段模板重述现值,编辑历史
即存档)。**热文件串行队是正文具名段**(文件 → 有序卡片清单 → 每张卡认领的区域;区域写不清就只
能整文件串行)。无心跳,活性惰性判定(只在接管冲突时评估,会话座位 >24h 无产出可回收);每轮巡检核
对自己的贴;竞态(空缺争用)三招 —— 动手前重 fetch 正文、审计评论时间戳先到先得、写后回读。状态
词表、接管/退场收尾清单等细则见 `references/seat-post-protocol.md`;epic 委托不入座位贴体系
...
仓没有车道标签,这形状是它们队列卡的常态);③ 有 `domain:*` 无pm-state。②③ 只取 `updated_at` 早
于 ~2 分钟的卡(半标注的第二来源是一次标签写入把老卡打成半标注),且不是可选项 —— 只带路由或状
态机其一的卡对两个视图同时不可见,而半标注卡是协议自己按设计生产的。排除:`tracking`、
`status:parked`(其正常形状恰是「带域标签无 pm-state」)、全部 `pm:seat` 贴;存量大时每轮限量、
优先最新。

**分类动作**(每张三选一 + 一个修复通道):**`pm:queue`** —— 有具名落点或复现的具体缺陷、范围明
确的工具/门禁修复、恢复不变量的 finding、test-only pin,无可问之事; **`needs-user-decision`**
—— 设计卡、feature/契约形状提案、需要 appetite 的多周程序、碰存量数据迁移形状或删除已发布能
力的,落卡时**必带四棱卡面块**(见「升级与决策」),⛔ 不留待有人接手再补;**`finding`** —— 观察
类(死代码、未演练漂移、抛光;真实但今天没有用户撞上),持有态待定级;**Repair first** —— 正文
被 sanitizer 截断的卡不可派发,评论修复指令后跳过;它是停摆指令,判据必须比其它分类更硬(双读取
),事后证伪要在同处公开作废。

**路由即分诊的技术判断,永不升级「哪个仓?」。** 读三仓代码定落点;跨仓的按contract-first 拆分;
父单已有子结构的,父单队列标签即可(sub-issue 自动成为候选),分诊时逐个展开路由、
补 `Blocked-by:` 排序,父单是协调节点**永不派发**。每张留一条英文审计评论(「Triage: lands in
…; rationale: …」),可选带 `Size/model suggestion: <S|M|L>, <sonnet|opus|fable>` 行供执行席参
考。跨仓查重(shadow 检查):跟跨仓引用 +关键词搜两姊妹仓 —— 在飞 ⇒ `Blocked-by:` 不派;open 未
认领 ⇒ 先收敛成一个派发入口;已完成 ⇒ 卡可能过期。**「生产者在哪?」是常设分诊问题**:declared
≠ enforced 形状的卡先问谁在写这个字段 —— 答案通常就是根因,并在派发前改变卡的范围与域标签。

**发现分诊轮(队列的出水口)。** 触发:每 ~5 轮,或 `finding` 存量 >~15 插队。对每条先做过时前提
检查(main 一天 ~18 合并,发现的准确性半衰期以天计),然后三选一:晋级(换 `pm:queue`)/ 关闭 not
planned(附理由,列入轮次报告 —— 维护者可否决重开,PM 不等批准)/ 持有(记一行为什么还留着)。**判
级发生在这里,不在立单时**(立单的 dev 只有局部视野,照实立单不自行压级)。**sweep 打包晋级**(维
护者 2026-08-07 批准)五条:同类才打包(全部命中同一判据);逐项清单(落点 | before | after)
是 PR 正文必备件; N 项之外零改动且 PR 自证;范围外发现照旧单开;sweep 卡本身是认领对象,成员单
保持标签但不再是可派发候选。
...
的。拆分/分票时 `target:*` 随工作走,对每一半重跑二元判据,默认继承,不继承要把理由按四类写在卡
上。**每个 backlog 恰好一个生产者**(主仓 → 分诊座位;objectui / cloud → 各自整仓座位),其余座
位不打不摘,⛔ 谁都不扫别人的 backlog;维护者的发版清单 = **三仓三条查询之和**,「归零 = 可发版
」指三张板都空。鲜度与发现分诊轮同节奏;**发版时刻 = 清板不是重扫**(修掉 / 摘牌 / 明示接受带
病发布三选一);**发版前置**:清板动手前取一次 pin 读数,pin 滞后 ⇒ console bump 单已存在且已上
板或明示接受,否则不 cut。
...
- **置信门(全部成立才可代裁)**:① 四棱同向(全指向同一选项,任何分裂即升级);②不在人工地板上
  ...
  换 `needs-user-decision` 为工作态 → 卡上贴四棱块 + 结论
  + `auto-adjudicated` 标记 → 轮次报告设**代裁清单**专节(聚合漂移的刹车);⑤ 代裁分析跑
    在 `claude-fable-5`(与维护者手工流程同档)。
...
**整车道一次读全,本地求交**(维护者 2026-08-11 接受):`list_issues` 带
`labels: [domain:X]` + 最小字段一次拿回全车道 open 集,各状态本地求交 —— 按单一 pm状态切片的
查询看不见其它状态,是结构性盲区。候选 = open、未 assign、无`needs-user-decision`;已排队父单
的 open sub-issue 自动是候选(`pm:epic` 父单的子树除外)。**每张候选读全文 + 全部评论**(裁决落
在评论区,跳过评论就是跳过裁决;评论还可能记着一半工作已交付);**派发前做 stale-premise
check**(issue 描述的是立单当天的仓 —— `git log --oneline -20 -- <paths>` 花几分钟,不查则赔一
次 agent-run)。

**批次独立性。** 一批内任两单不得可能碰同一个包/registry/barrel/spec schema;拿不准就串行。**
同文件单跨轮硬串行;延后不是搁置**(被延后那一刻就把已知的坑记到该 issue上)。**维护者明示豁免
同文件串行时,替代纪律四条**(豁免的是排队,不是防撞):① 认领评论把文件面申报到**区域**级(函数/
段落);② 开 PR 前合一次 main;③ 兄弟卡落地后再合一次;④ 冲突交合并队列仲裁,⛔ PM 不手动排序;豁
免不外溢,无明示豁免默认硬串行。**阻塞解除后给延后单重新定价**:派发前一单时带必答项「你的改动
让 #X 变简单/变难/变得不必要还是无影响?」,派发后一单前用这个回答重读它的选项与成本,⛔ 不沿用
立单时那份(适用判据:前后单共用同一契约或数据表示)。

**选择优先级**:`priority:p0` 插队 > 解锁扇出 ≥2 的上游单 > `target:` 板上项 > 被任一开
卡 `Blocked-by:` 点名的上游单 > 普通队列项(维护者 2026-08-11 裁定);扇出从`Blocked-by:` 反向
索引现算,⛔ 不为此发明存储标签;**优先是排序,不是豁免**。**解锁那一刻两类断言同时最不可信**:
...
评论承载身份;跨账号只需一条:assignee 不是你 ⇒ 永不碰。每张选中的单,派发前按序执行**一个原子
对**:
...
**Model tiering(维护者 2026-08-10 三档裁定;本节覆盖一切更旧档制)。** 授权原话:「项目经理技能
还需要考虑的是派任务时使用什么模型,也应该项目经理决定,最低下限sonnet,最高可以 fable」;「比
如 更新 项目经理技能 必须要使用 Fable 5」。政策:**派发模型是 PM 的逐卡显式决定** —— 下
限 `sonnet`(机械卡:正确性由门禁农场机械判定,失败模式是漏跑门不是判断失误);默认判断
档 `opus`(M/L、裁决实施、多面语义、任何带设计判断的卡;拿不准就升一档 —— 错派低档的返工贵过省
下的额度);上限 `fable`(交付物本身是协议/流程/编排结构的最重卡,按卡取用不是新默认)。**⛔ 强制
条款(无向下裁量权):凡改 `.claude/skills/pm-dispatch/**` 的卡,一
律 `model: "claude-fable-5"` 派发。**档位**逐次派发显式传参,永不省略**(解析顺序:环境变
量 → 派发参数 → 定义frontmatter → 主会话模型;pin 只管什么都不传的情形;被允许名单挡下时回退**
继承的模型**,正是 pin 要防的静默继承),连同尺寸写进认领评论「Container & model」行;分诊
suggestion 行是输入不是决定,不采纳给一句理由。
...
- **三分区,措辞决定 dev 敢不敢证伪**:「裁决(不可重裁)」执行不重开;「PM 机制假设(须实测,鼓励
  证伪)」dev 动手前验证,证伪了照实报告并按裁决意图换路径;「PM 建议的路线(可选,实测优先)」有
  更好的就换。凡「我觉得可以这样」的一律降到第三块 ——机制假设穿着裁决的衣服,两个方向都是返工
  。
- **标准非协商条款 ⛔ 不抄进派发词** —— 已下沉进 `.claude/agents/os-dev.md`;无条件条款只能住
  在角色文件(派发词与它冲突时它胜),错了就修那里,⛔ 不靠派发词临时覆盖其默认值。**清单、路径
  、行号在派发那一刻从树上取**,⛔ 不从卡片/上次派发/记忆抄(取数
  `node scripts/pm/dispatch-gates.mjs <改动路径>…`;取数的是 PM 不是 dev —— dev 只跑被点名的
  门禁族,全 farm 归 CI);行级断言转述进派发令前必须自己重验。
...
**资源与后端。** 并行 agent 共享一个容器,内存峰值来自 build+test(os-dev 定义已强制验证锁
/heap 上限/scoped filter/收尾清理);build 重的降 `batch:2` 或走`mode:cloud`;OOM 死的单独重派
。**M 及以上默认云卡**(维护者 2026-08-10 裁定,原话:「确认,M 及以上默认云卡」):M+ ⇒
`mode:cloud` 独享容器,⛔ 不混共享批次;只有 S 级**机械**卡留 `mode:subagent`;S 级但不机械(判
断面在设计不在门禁)按 M 待遇 —— 尺寸不是档位的充分判据。判定连同档位写进认领评论。

**云卡四课**(细则见 `references/dispatch-runbook.md`;一次性云卡用`create_session`,⛔ 不
用 create_trigger+fire —— 维护者 2026-08-07 拍板,trigger 流只留给定时/重复型):① 授权面
随 source 不随环境(带 `source_url` + `outcome_branch` +显式 `model` + 车道名开头
的 `title`);② 派发词必带**自驱条款**(⛔ 不为提问/中期汇报结束回合;合法回合终点只有推送完
成 + 终报,或硬阻塞详报);③ 交付通道 = dev 自开draft PR + 终报以 issue 评论交付;④ **云卡 draft
PR 一存在立即`subscribe_pr_activity` —— 硬步骤**(未订阅的云卡 PR 是轮询负债)。CCR 云会话不注
册跨会话消息 roster —— `SendMessage` 直投 not-reachable 是设计而非故障(维护者2026-08-11 裁定
:事件驱动架构即长期方案),⛔ 不复测 roster 路径。**接手中断的dev**:先试 SendMessage 复活
,resume 不可用才走 worktree 接手协议(细则同上表)——⛔ 不重跑原派发词;worktree 不新建;
逐 hunk 读既有工作再决定;验证完整重跑; assignee/认领评论/分支全不动(既有认领的延续,不是新认
领)。
...
- **直接验收兜底(报告丢失 ≠ 验收停摆)**:(a) draft PR 在且 CI 全绿 + (b) 探活确认已死
  或 ≥2h 无推送 + (c) 报告未达 ⇒ 直接按 PR 验收(逐文件核对 diff 与申报文件面,对
  照 `origin/main` 复核前提与验证叙述,复核判据不减);先探活后翻 ready。舰队级死因(全账号断粮)
  下取 (a)+(c) 照常收口,PR body 就是报告。
...
卡 —— 从 `Blocked-by:` 反向索引现算);**finding median age**(老化 = 发现分诊轮欠账);**release
blockers**(三仓 `target:<major>`之和,归零 = 三张板都空)。

**波次收工点(维护者 2026-08-09 裁定)。** 班内节奏:一波处理完 → 收工存档 → `/compact` → 再继
...
钟(有在飞收紧回 ≤45),待命五项 —— findings 配合(补证据、上报异议)、`pm:blocked` 解锁扫描(含回
...
**落卡/升级流程**:① **先刷新卡片前提** —— 隔夜没动的卡默认按「前提未经验证」处理; **卡上每条
前提行自带一条 re-check 命令**(`git log … -- <path>`、REST compare、带引号精确名 grep、
`ls-remote | grep <branch>`),复升级时逐条**跑**一遍,零命中/变形的就地改写或撤卡。② 决策默认
**锚在所属 issue**(分析发评论、挂`needs-user-decision`、退出活动队列;标签就是维护者的收件箱,
答复后摘标回队);仅当决定无自然锚才单开 `[Decision] <一句话>` 卡。③ 分析(英文)含:背
景 / 带 re-check命令的前提 / 具体问题 / 选项 / 推荐 / 相关单与 PR。
...
- Every dev agent works in its **own worktree per repo**(hook 强制;os-dev 定义重申);并行度
  ...
  一个车道认领**(唯一例外:分诊指定的跨域例外单);**`domain:*` 只有一个生产者**,执行座位不改标
  签只上报误标。
