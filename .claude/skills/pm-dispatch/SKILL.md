...
无角色参数时按执行姿态运行,车道由座位贴认定。角色参数的意义:座位 Routine 的prompt 收敛
成一行
技能调用,座位行为全部版本化在 git,升级走技能 PR,⛔ 永不靠重建Routine 改行为。**探针先
行(启动
...
UI 创建、UI 钉模型 —— 凭据随 Routine 所挂 repo sources 走,Routines UI 并无 GitHub 连接器入口
(2026-08-23 实测;会话内 create_trigger 建的是静默零产出陷阱,细节在 references);② 创建后手动
fire 一轮烟测,判据取**GitHub 上的产出**;③ 每 fire 一轮:读座位
贴 → 从 labels 重建状态 → 跑一轮 → 结束,不继承上下文。**开轮互斥读法**(宁少跑,不让两
个同座位会话并行写标签):fire 开始、首个写动作之前读四个读数,任一未满一个轮次 ⇒ 自
退 —— 最近收班简报;晚于它的最新「开轮」标记(轮中会话在座位贴上唯一的痕迹);本车道
`pm:dispatched` 卡最新非本 session 的 `Claim:` 评论(拼写见「认领」,车道盘点已取);本车道
`state=CLOSED` 按 `updated_at` 降序第一张卡的 `Claim:` session ID —— 第四读免疫标签惯例:不用
`pm:dispatched` 的会话对前三读完全隐形,叠加过期座位贴后与空席同形(实测:一个 16 分钟前还
在一趟合并五个 PR 的会话被读作「清」)。标记可漏写,认领不可省;第三、四读数只**增
加**自退、永不清座位;两读按 session ID 判「非本 session」是特意保留的**安全方向**:`mode:subagent`
的 dev 认领带派发会话自己的 ID、读作本会话在飞,不触发自退 —— 同会话的 dev 本非互斥对象,
分辨「谁」靠分支(见「认领」)。四读数皆清,先在座位贴留一行开轮标记(session ID + fire 时刻)再跑轮。
...
- **写后回读按风险定向;标签写恒为硬步骤 read-modify-write + 回读**(维护者 2026-08-18 裁定):①
  ...
  盖,不只防 sanitizer);「API 返回 200」≠「落地内容正确」。
- **PM 写进 GitHub 的文本 ⛔ 不用尖括号路径占位符**,改写成「后跟显式路径」的说法 ——
  ...
  护身符。不产生这个构造才是修法,回读只是兜底:必须携带易损片段的评论(派发令与裁
  决,不含例行标记)post 后读回。
...
- **GitHub 限流 ⇒ 先 `GET /rate_limit`(零配额)取精确 reset 与余量,⛔ 不盲等**;公开仓降级 WebFetch
  读法与限流细则见 platform-readings。
...
- **放行认门禁 job 的结论,不认聚合读数**(承载门禁族的 job 须 `completed: success`,`in_progress`
  不算;聚合只作阴性筛查,放行按名定向读单条 job,⛔ 不拉全表);advisory 门禁红着合并进
  main 是共享损伤,任何车道发现都立即止血 + 立单(main-red 跳队与锚卡两约定见
  landing-operations B);落地两个读数与踢出重投的签名纪律见事实表。
...
次;授权面 = 分诊本身(定级/路由/标签/既有评论格式),⛔ 不写码不认领;产出落卡,与 Routine
...
每轮解锁扫描,可执行判据由分诊 Routine 每日一个低频子轮批量执行(十几条一行判据逐小时
...
2. **认领评论**(英文),固定形状;session ID 不可省,但它分得清什么随模式而变(实测三次:`mode:subagent`
   的 dev 就在 PM 会话内,与 PM 两条认领的 session ID 逐字相同)—— subagent 下认领的甄别身份是**分支**,
   两条评论买到「确有 dev 跑过、在哪条分支」;`mode:cloud` 的 dev 自有会话,才额外买到「哪个会话」;
...
执行 PM 席默认 `opus` 会话,fable 保留给分诊 Routine(代裁 + 契约复审)、本席与条款②工作 ——
各自档位与 Routine UI pin 归既有条款(本行只记策略)。降档出口两条:**额度耗尽豁免**(维护者
2026-08-13
...
- **派发令必须要求 dev 留自己的认领评论**(其**本身**的 session ID + 分支,在 PM 那条**之外**)——
  共享身份下 assignee 只说「有人认领了」,分得清「谁」的只有评论;两条评论各买到什么、subagent 下
  甄别靠分支,见「认领」规则 2。「已认领、别动 assignee」⛔ 收窄到**恰好 assignee 这个字段**:它读起
  来像「认领协议已满足」,那是另一句话且可能是假的(实测:被这么告知的卡当时无 assignee、无认领评论、无分支)。
...
- **检测**:`ccusage blocks` 只有单容器视野、估成本不估套餐余量(账号级剩余额度无接口可查);
  **权威的墙信号是失败本身**——撞墙报文里的重置时刻在那一刻可得、事前查不到,把它
  记下来(工具读数与盲区在事实表)。
