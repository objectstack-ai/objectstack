...
- ⛔ 分诊席永不认领、永不派发、永不写代码;执行席只在本车道认领,永不改 `domain:*`。
...
- 读数三:本车道 `pm:dispatched` 卡最新非本 session 的 `Claim:` 评论,只计认领本车道派发的。
- 读数四:本车道 `state=CLOSED` 按 `updated_at` 降序第一张卡的 `Claim:` session ID。
- 评审链与总监席落在车道卡上的笔迹是跨席作业,不算占席。
- 读数三、四只增加自退、永不清座位。
...
- 每批先过时前提检查再三选一:晋级 / 关闭 not planned(维护者可否决重开,PM 不等批准)/ hold。
...
- 每张板恰好一个生产者。
- 鲜度节奏、清板三选一、pin 前置细则见 `references/seat-post-protocol.md`。
...
- 并行度以 `batch` 封顶,同批按构造文件面不相交;`priority:p0` 插队可超 `batch`。
...
- 维护者明示豁免同文件串行时,替代纪律四条:① 文件面申报到区域级。
...
- 取卡全序:`priority:p0` 插队 > `pm:blocking` > `target:` 板上项 > type `Bug` > 其余;同级按卡龄。
- `pm:blocking` 级内先按解锁扇出(从 `Blocked-by:` 反向索引现算,⛔ 扇出数不落标签)。
- 全序每级取既有信号现读/现算,零逐卡维护;优先是排序不是豁免。
...
- 文件面对跨域例外单必填、普通单建议;分支名必须带 issue 号。
...
- 门禁清单用 `node scripts/pm/dispatch-gates.mjs` 现取,取数的是 PM。
...
- `premise_still_valid: false` + 无 PR 是合法且常常有价值的交付。
...
- 触 `skills/**` 的 PR 加问整包价值密度:从整包加载的客户 agent 的座位上读。
- ⛔ 不从功能作者的座位读价值密度。
...
- 或者:修复需要破坏性/难回滚动作。
- 其余都是 PM 的裁量:裁定、派发、给维护者否决窗口而非许可门。
...
- ③:答复/代裁到手,裁决记录四件同笔。
- 四件 = 鲜度门 / 状态转换同笔 / `Blocked-by:` 活性现验 / 条件已判即判。
...
- `out_of_scope_findings` 应已由 dev 立成无 assignee 的卡:查重先行、归挂判据、立在修复落地仓。
- 范围外发现的标签按种分:观察类打 `finding`,具体缺陷不打标留分诊。
