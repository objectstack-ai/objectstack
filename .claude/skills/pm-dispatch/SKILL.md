...
- 发布窗口「已上膛」判据与其真读法住 `references/dispatch-runbook.md` 落卡与裁决记录细则。
...
- 一轮 = 从 fire 醒来到队列清空或用量窗口耗尽;轮次报告是轮末汇报,⛔ 不是停止信号。
...
- 零命中须用确定存在的邻近词反查;控制词须与主张同主体,否则该零作废,不是阴性。
- 同主体 = 同语料、同路径形、同包界、同引法、同失效形态:跨行短语配跨行控制词。
