...
- **创业阶段不扩散需求** — **创业阶段聚焦原则**:能力扩张默认从紧,无拉动的声明面按
  implementation-first 处置,已发布零消费能力不因沉没成本豁免。
  **过渡也从紧 —— 创业阶段不渐进**:废弃别名/拼写与能力退役默认**立即退休**,无分阶段
  窗口、无双拼写宽限;staged 选项仅凭具名外部用户证据方可荐。**新增门禁默认否**:门禁是
  只减不增的零件,例外只有维护者点名;荐新增门禁须引其点名原话,否则荐「不加」。
