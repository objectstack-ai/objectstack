...
not-reachable 是设计非故障(维护者 2026-08-11 裁定),⛔ 不复测。**接手中断的 dev**:先试
SendMessage 复活,resume 不可用才走接手协议 —— ⛔ 不重跑原派发词;worktree 不新建;逐 hunk 读
既有工作再决定;验证完整重跑;assignee/认领评论/分支全不动(既有认领的延续,不是新认领)。
...
- **两级信号**:权威的**预警**信号是 `rate_limit_info`(`status: "allowed_warning"`,如 seven_day
  周窗)⇒ 当轮起进保守派发姿态(细则见 references);权威的**墙**信号仍是失败本身 —— 撞墙报文里
  的重置时刻在那一刻可得、事前查不到,把它记下来。`ccusage` 只有单容器视野、估成本不估套餐余量。
- **跨墙定时器**:拿到重置时刻 ⇒ 一发定点 `run_once_at`(reset + 缓冲)优先,多天窗口只用这一发、
  ⛔ 绝不挂跨多天的每小时 cron;5 小时级没拿到 ⇒ 挂每小时 cron,⛔ **不用 send_later 链** ——
  run-once、fire 后自禁用,死窗口那一发是否被平台重试未实测且文档未承诺,一次性链条可能恰好断在
  它存在的意义上;第一枪成功的 fire 跑恢复后**删除 cron**(孤儿定时器)。fired 文本照定时器写法纪律。
