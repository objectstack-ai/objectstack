...
- **spec 改动的 fixture triage 必须跑消费包测试**(A 包的改动可让 B 包的 fixture 反着断言,
  spec 内的 sweep 看不见)—— 动契约面的派发令点名消费包测试清单,报告要各消费包真实读数。
...
状恰是「带域标签无 pm-state」)、全部 `pm:seat` 贴;存量大时每轮限量、优先最新。
...
被 sanitizer 截断的卡不可派发,评论修复指令后跳过;停摆指令判据必须比其它分类更硬(双读取),事
后证伪同处公开作废。
...
检查(发现的准确性半衰期以天计),然后三选一:晋级(换 `pm:queue`)/ 关闭 not planned(附理由,列入
轮次报告 —— 维护者可否决重开,PM 不等批准)/ 持有(记一行为什么还留着)。**判级发生在这里,不在
立单时**(立单 dev 只有局部视野,照实立单不压级)。**sweep 打包晋级**(维护者 2026-08-07 批准)五
条:同类才打包(全部命中同一判据);逐项清单(落点 | before | after)是 PR 正文必备件;N 项之外零
改动且 PR 自证;范围外发现照旧单开;sweep 卡是认领对象,成员单保持标签但不再是可派发候选。
...
个 backlog 恰好一个生产者**,⛔ 谁都不扫别人的 backlog;「归零 = 可发版」指三张板都空。鲜度同发
现分诊轮;**发版时刻 = 清板不是重扫**;清板三选一与 pin 前置细则见 references。
...
**每轮整车道重读、一次读全,本地求交**(维护者 2026-08-11 接受):`list_issues` 带
`labels: [domain:X]` + 最小字段一次拿回全车道 open 集,各状态本地求交 —— 单一 pm 状态切片的查
询看不见其它状态;⛔ 不拿上轮/检查点清单当派发输入(实测两次漏卡)。候选 = open、未 assign、
无`needs-user-decision`;已排队父单的 open sub-issue 自动是候选(`pm:epic` 父单的子树除外)。**每张候选读全文 + 全部评论**(裁决落在评论区,跳过评论就是跳
...
文件串行时,替代纪律四条**(豁免的是排队,不是防撞):① 文件面申报到**区域**级(函数/段落),同一
热文件并发编写者设上限;② 开 PR 前合一次 main;③ 兄弟卡落地后再合一次;④ 冲突交合并队列仲裁,
⛔ PM 不手动排序;豁免不外溢,防的是并发**编写者**不防队列里的成品 PR —— 前一 dev 会话一结束即
可派下一张同文件卡,⛔ 不等其 PR 落地。**
...
卡 `Blocked-by:` 点名的上游单 > 普通队列项,项内 `Bug` 先于 `Feature`/`Task`(维护者 2026-08-11
与 2026-08-12 裁定)—— **平手判据,⛔ 不是优先级阶梯**(P1–P5 细粒度渐变已试已拒:没人维护必烂)
;扇出从`Blocked-by:` 反向索引现算,⛔ 不为此发明存储标签;**优先是排序,不是豁免**。**解锁那一刻两类断言同时最不可信**:
...
向下裁量权)两条**:① 凡改 `.claude/skills/pm-dispatch/**` 的卡一律 `model: "claude-fable-5"`;
② 凡**改变契约接受/拒绝行为或扩大公开面**的卡(`domain:spec` 语义车道;判据即分诊代裁的机械边
界测试与一包三席分席判据,⛔ 不另抄第二份)一律 `claude-fable-5`(维护者 2026-08-12 裁定,原话
:「同意,就按语义面收窄,立卡并通知 spec 席」)—— 错在唯一契约上毒化一切下游,是全仓最贵的错误
类。明确不变:`spec-surface` 照旧 sonnet/opus;`spec-tooling` 默认 opus,门禁语义设计时升 fable
;下游适配卡(消费侧清扫)默认 opus 拿不准升档照旧。**档位逐次派发显式传参,永不省略**(解析顺序
与 pin 语义以 os-dev 定义 frontmatter 注释为权威;被允许名单挡下时回退**继承的**模型,正是要防
的静默继承),连同尺寸写进认领评论「Container & model」行;分诊 suggestion 行是输入不是决定,不采纳给理由。
...
- **翻转公开语义的裁决随卡带全仓 pin 清扫**,两句缺一不可(全仓一轮翻完;翻转后的pin 断言新语义
  ...
  义 ⇒ 编译面清单逐面申报(⛔ 静默略过);测试进 type-check 债包 ⇒ 点名
  `check:type-check-debt`(债包 `pnpm typecheck` 结构性全绿)。
...
source 不随环境,且 **push 授权 ≠ GitHub API 可达** —— App 未接通的 org 里 dev 推得了分支、开
不了 PR 读不了 issue:完整令随 prompt 携带 + 发布失败回落条款,回落协议见 runbook;② 派发词必
带**自驱条款**;③ 交付通道 = dev 自开 draft PR + 终报以 issue 评论交付;④ **云卡 draft PR 一
存在立即 `subscribe_pr_activity` —— 硬步骤**。云会话 `SendMessage` not-reachable 是设计非故障
(维护者 2026-08-11 裁定),⛔ 不复测。**接手中断的 dev**:先试 SendMessage 复活,不可用才走接手
协议(四条增量见 runbook;既有认领的延续不是新认领,⛔ 不重跑原派发词)。
...
- **停摆永不自愈**:携带任务中途状态的完成通知就是停摆信号,立刻 SendMessage 附前台执行姿态句
  ,⛔ 不等静默阈值(阈值是给「没有回答」的);复位走梯度、逐次更具体,第三次停摆判 unreliable 按
  接手协议重派。这里是消费侧兜底,⛔ 不能写成「派发词写全了就可以不探」。
...
- **直接验收兜底(报告丢失 ≠ 验收停摆)**:(a) draft PR 在且 CI 全绿 + (b) 探活确认已死
  或 ≥2h 无推送 + (c) 报告未达 ⇒ 直接按 PR 验收(复核判据不减,细则见 runbook);先探活后翻
  ready;舰队级死因(全账号断粮)下取 (a)+(c) 照常收口,PR body 就是报告。
...
- **碰生成物的 PR,入队前先同步 + 整体重生成** —— os-regen 驱动会零冲突标记地**静默丢掉一侧改
  ...
  merge 再重生成**,顺序陷阱见 landing-operations);重生成后断言兄弟单条目与上一单**实现体**仍
  在;清单当场读 `grep os-regen .gitattributes` ⛔ 不抄进派发令当常量;⛔ 禁止为凑相等手改锚点。
...
- **检测**:`ccusage blocks` 给窗口边界与燃烧率,但单容器视野、估成本不估套餐余量;**权威的墙
  信号是失败本身** —— 撞墙报文里的重置时刻在那一刻可得、事前查不到,把它记下来。
- **跨墙定时器**:拿到重置时刻 ⇒ 一发定点(reset + 缓冲)优先;没有 ⇒ 挂每小时 cron Routine,
  ⛔ **不用 send_later 链**(run-once、fire 后自禁用,一次性链条可能恰好断在断粮窗口里 —— 论证
  见事实表);第一枪成功的 fire 跑恢复后**删除 cron**(幸存 cron 是孤儿定时器)。fired 文本照定
  时器写法纪律。
