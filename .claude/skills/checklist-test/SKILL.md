...
- **标题 —— 恰好一种形状,严格契约。** 维护者裁定 2026-08-20:收紧成单一形状,
  `scripts/qa/qa-rollup.mjs` 按此**严格**解析(⛔ 不是宽容解析)。形状:

  `QA run · <selector> (<已判定>/<总数>) · <framework-sha8> · <YYYY-MM-DD> · <counts>`

  - 恒为 **5 个 `·` 分隔字段**,不多不少。
  - `(<已判定>/<总数>)` **强制**,纯数字、无字词,`已判定 ≤ 总数`。⛔ 以下写法全部**退
    役**,一个都不许再用:`(FULL area)`、`(77 items)`、`(5 of 10 items)`、
    `(6/14 items consulted)`,以及整个省掉括号。`(FULL area)` 正是本裁定的由来 —— 它不
    声明总数,而戴着它的一条记录只判了本区 33 项里的 13 项,读起来却像全覆盖。
  - `<framework-sha8>` —— 恰好 8 位小写十六进制。
  - `<YYYY-MM-DD>` —— 真实日历日期。
  - `<counts>` —— ` / ` 分隔的 `<n> <VERDICT>` 段:判定词**大写**,按
    `PASS / PARTIAL / FAIL / BLOCKED / NOT-RUN` 的顺序,每桶至多出现一次。NOT-RUN 只此
    一种拼法;⛔ 尾随的 `(11 not-run)` 括号写法已退役。**省掉某桶 = 未声明,不等于 0**
    —— 要表达零就写 `0 FAIL`,roll-up 把两者显示成不同的东西。
  - 那个分数是标题的一部分,不是可选装饰。
  - **存量记录不迁移**(裁定:new records only)。执法面是 roll-up 自己:
    `node scripts/qa/qa-rollup.mjs` 把契约生效日之后写的、不合形状的记录单独列进
    **Not parsed** 段并点名出错字段 —— 偏差第一次出现就可见;生效日之前的记录另列一段,
    标明「预期之内、非缺陷」。写完标题后跑一次它,是这一步的自检。
...
- **完成判据** —— 一个区只有在**每一项都有判定**时才算跑完。跑不完的一轮照常立单:
  ...
  `not-run` 是诚实判定 —— 判了 1 项、11 项没跑,就写 `(1/12) · … · 0 PASS / 1 PARTIAL
  / 11 NOT-RUN`。标题现在没有别的写法可选:`(FULL area)` 那类不声明总数的措辞已退役,
  正因为它把这种一轮读成了全覆盖。
