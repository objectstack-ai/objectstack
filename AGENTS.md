...
agent/task** (`git fetch origin main && git worktree add ../objectstack-<task> -b
<branch> origin/main`; run `pnpm install` in the new tree) so file systems are
physically isolated — mandatory, not a preference (Prime Directive #11), hook-enforced.
Working in the shared `main` checkout is *not* a supported fallback: branches get
switched and shared files — including ones you just wrote — get reset *under you*
mid-task (full sessions of work were silently reverted before the rule was enforced).
