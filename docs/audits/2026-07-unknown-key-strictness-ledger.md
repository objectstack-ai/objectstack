...
waves met, because it is the failure mode this table is most exposed to:
each PR deleted its own rows and decremented this header by its own count, so
git merged the ROWS cleanly (they do not overlap) and left the header
conflicted — and the subtotal line below it, which conflicts with nothing,
merged clean while being **wrong on every branch**. No number was a
...
favour of any side, and `check:strictness-ledger`'s header arithmetic is
what settles it. A clean-looking merge here is not evidence of anything.

**批 12** then did something none of the earlier waves did: it closed
`etl.zod.ts`'s authorable half (7) and left the row **shrunk but alive** at a
wire floor of 3, rather than reaching zero. That exercises the half of the
bookkeeping the reverse pin cannot reach. The pin proves a row's work is
*done* — it fires when a file hits zero — and it is completely silent about a
row whose work is *deliberately partial*. To this gate, "finished, 3 wire sites
remain by decision" and "nobody got to it" are the same table. Only the `Class`
column separates them, so from 批 12 on that column is load-bearing rather than
descriptive; and because a reader who arrives via the code will never open this
file, the same decision is written on the schema in `etl.zod.ts` and pinned in
`etl.test.ts`.

**Authorable strip in `automation/`: 9 of 35** (was 41 of 67 before the three
waves). What remains of the ruling's "known main body" is `flow` 6, and one
each from `flow-function` / `time-relative-trigger` / `webhook`. ⚠️ 批 11 is
still in flight against exactly those rows, so expect this paragraph and the
section header to conflict again; resolve it the way this one was resolved.
