...
**Authorable strip in `automation/`: 32 of 58.** This is the ruling's "known main body"; batch 11 took 9 of it (the six nested `flow.zod.ts` blocks + `time-relative-trigger` + `flow-function` + `webhook`), and three files reached zero strip sites, so their rows are gone — the reverse pin above did the deleting, not a memory.
