...
- **`no gate`** — ~~`chart.zod.ts`'s remaining pair from 批 15~~ **left this
  ...
  `ChartAggregateSchema` instead of re-deriving it; the pair is `authorable`
  again and its strictness half is #5583. Still here: **all of
  ...
  site being closed. Note what #5020 makes visible about the class as a whole —
  ...
  rejection nobody had yet seen a gate produce.
