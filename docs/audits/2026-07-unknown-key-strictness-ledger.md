...
| `control-flow.zod.ts` | wire | **out of scope** — the file's one strip site is `TryCatchErrorValueSchema` (+1 at #13681), the value the ENGINE binds to a `try_catch`'s `errorVariable` (default `$error`) when its catch region runs: engine-emitted, read by the catch region's expressions, authored by nobody — the same `wire` verdict `execution.zod.ts`'s run-summary family carries, for the same reason (closing it turns a future engine-side key into a parse failure with no author to prescribe to). Deliberately a plain `z.object` rather than `strictObject`: the unknown-key prescription an authoring surface owes has nobody to address. The five authorable sites (`FlowRegion` / `Loop` / `ParallelBranch` / `Parallel` / `TryCatch`) stay strict as of #4001 批 10 — this row reopens none of them, and its strip count is a floor, not work |
