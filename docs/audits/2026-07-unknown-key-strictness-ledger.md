...
- **`no door`** — ~~`touch`, `animation`, `dnd`, `keyboard` and `offline` from
  批 13~~ **RETIRED at #4988** (all five files deleted; see the destination note
  under the `ui/` triage table); `sharing.zod.ts`'s `EmbedConfig` and
  `notification.zod.ts`'s `NotificationAction` from 批 14 (#5015);
  `widget.zod.ts` plus `i18n.zod.ts`'s remainder from 批 16 (#5055).
  批 13's five are the first `no door` cohort to reach a verdict, and the
  verdict was REMOVE — which is what this class was for.
