...
it is the maintainer's call, and `verify` — "held pending a check" — is the one
...
- **`no door`** — `touch`, `animation`, `dnd`, `keyboard` and `offline` from
  批 13; `sharing.zod.ts`'s `EmbedConfig` and `notification.zod.ts`'s
  `NotificationAction` from 批 14; `widget.zod.ts` plus `i18n.zod.ts`'s
  remainder from 批 16 (#4988, #5015, #5055).
