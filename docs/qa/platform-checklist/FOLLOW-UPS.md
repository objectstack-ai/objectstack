...
| "a second signed-up (non-admin) user in seeds" | both demo personas get a better-auth credential account at boot (`seed-approval-demo.ts` → `ensureCredentialAccount`, keyed on `(provider_id, account_id)` through better-auth's `internalAdapter.createAccount`). ⚠️ This row used to add that the personas' issuer was derived from the dev admin's own row; withdrawn 2026-09-23 with #19217 — better-auth 1.7.3 rolled the issuer-scoped account identity back and `sys_account.issuer` retired with it (`packages/spec/src/migrations/entries/semantic/18.sys-account-issuer-retired.ts`), so the seed derives and stamps no issuer. The three items this row unblocked withdrew the same clause with #17453 | `approvals.per-group-signoff`, `approvals.viewer-gating-submitter-side` (was `blocked(fixture)`), `approvals.ooo-delegation-reroute` |
