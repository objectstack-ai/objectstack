 ...
 * receipts and `expand`/`select`, and the ADR-0049 `apiEnabled` / `apiMethods`
 * exposure gate `callData` applies before dispatch. None of those is the
 * authorization boundary — the exposure gate is a SURFACE-AREA control by its
 * own ADR note, and every call here still passes the engine's CRUD/FLS/RLS —
 * but the two transports should not differ at all, and unifying them behind one
