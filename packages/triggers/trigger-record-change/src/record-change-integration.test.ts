 ...
 * registers its own test object. `ObjectQL.probeInstallOrganizations` then
 * probes `sys_organization` on every write. That probe is fail-soft by
 * construction, so the read is EXPECTED — but the driver and the engine each
 * log the fault on the way out.
 *
 * ⚠️ [#17985] The five authz tables the resolver
 * (`core/src/security/resolve-authz-context.ts`) reads USED TO BE on this list.
 * They are not any more, because `attachSqlite` now provisions them (see
 * `authzResolverObjects` below) — the outcome this channel's own contract
 * names: "a table that started resolving means the fixture now provisions it".
 * Their refusal was never only noise: `tryFind` answers `[]` for a missing
 * table, so every write below resolved its grants from reads that never
 * happened. ⛔ The count fell because the reads SUCCEED, not because anything
 * was quietened.
 *
 * ⛔ This list is MEASURED, not copied from the probers' source: before this
 * file provisioned anything, a run at `logger: { level: 'info' }` emitted 84
 * `refused a read on '<t>'` driver lines and 84 matching `ERROR Find operation
 * failed` engine frames, split sys_user 15 / sys_member 15 / sys_user_position
 * 15 / sys_user_permission_set 15 / sys_position 15 / sys_organization 9. The
 * 75 authz ones are gone; the one channel left is the one declared below. A
 * read this file stops provoking still shows up as a CHANGED SET (red
...
async function attachSqlite(objectql: any): Promise<any> {
  ...
  // [#17985] Registered on the ONE seam every boot site in this file goes
  // through, before any caller's `syncSchemas()`, so each write below resolves
  // its grants from reads that HAPPEN rather than from `tryFind`'s empty answer
  // for a missing table.
