...
export const AuthPluginConfigSchema = lazySchema(() => z.object({
   ...
   * through the admin plugin's ban/unban (ADR-0134); without effective
   * SCIM, unset means off. An EXPLICIT `true` mounts the plugin
   * unconditionally. An EXPLICIT `false` declines the admin surface — and
   * because of the ADR-0134 coupling, `false` beside effective SCIM is a
  ...
  admin: z.boolean().optional().describe(
    ...
    'Unset: forced on by effective SCIM (ADR-0134), otherwise off. An explicit false beside ' +
   ...
   * ban/unban runs through it (ADR-0134). An explicit `admin: false` beside
