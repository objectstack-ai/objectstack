...
export const ActionSessionSchema = lazySchema(() => z.object({
  ...
  positions: z.array(z.string()).optional().describe(
    ...
    + 'on the #3280 → #3290 `session.tenantId` alias precedent path (deprecate, one window, remove). '
  ...
  roles: z.array(z.string()).optional().describe(
    ...
    + '`action-session-roles-to-positions`; removal follows the `session.tenantId` alias precedent '
    + 'path (#3280 deprecated → #3290 removed in v11). Never gate PRIVILEGE on this array — ask the '
