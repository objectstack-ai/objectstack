 ...
 * wrote. The pins below carry both directions of that arm, and the carve-outs
...
describe('#6227 — what stays accepted (the #5685 side: never stricter than the runtime)', () => {
  it.each([
    ...
    // A string operator carrying a number: no backend refuses it. (`icontains`
    // is the one exception and it is the TABLE's row, not an analogy — see the
    // comparand pins below.)
