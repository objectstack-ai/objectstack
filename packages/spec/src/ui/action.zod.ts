...
const actionObject = () => z.object({
   ...
   * declares warns with a did-you-mean, because nothing downstream catches it:
   * on a SQL driver the stray column fails the whole call with a driver-level
   * error far from the authoring site, and on a schemaless driver it is
   * persisted as an undeclared key (#4271). Advisory only, and blind to
