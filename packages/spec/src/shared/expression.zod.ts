 ...
 *
 * Returns {@link EvaluatedExpression}, not {@link Expression}: `source` is a
 * required parameter here and is always written to the envelope, so the wider
 * return type declared a shape this function cannot produce — a `source`-less
 * envelope. See {@link cel} for why a helper that over-declares its output is
 * a producer-side defect rather than a harmless looseness.
 */
 ...
 * ALWAYS sets `source` from the rendered template, so the wider return type
 * was a declaration of something the function cannot produce. A helper that
 * over-declares its output is a producer-side defect (Prime Directive #12)
 * even while nothing has yet asked for the narrower shape: every slot that
 * composes {@link EvaluatedExpressionInputSchema} — `FlowEdgeSchema.condition`
 * today — requires a non-blank `source`, so a ``P`record.status == 'paid'` ``
 * assigned straight into one is a TS2322 against a value that satisfies the
 * schema at runtime.
 * Fixed at the PRODUCER rather than by rewriting call sites: the return type
 * now states what the helper emits. Narrowing a return type removes nothing
 * from a caller — `EvaluatedExpression` is assignable to `Expression` — so
 * every existing consumer still compiles.
 */
