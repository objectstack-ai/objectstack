...
export const entry: SemanticMigration = {
  ...
  surface: 'dataset measure `aggregate` × `field` pairs (`DatasetMeasureSchema`, the rows '
    + 'inside `Dataset.measures[]`) whose aggregate the field\'s declared `FieldType` '
    + 'cannot carry — notably `avg` / `sum` over a `date` / `datetime` / `time` field, '
    + 'and `sum` over a `percent`',
  replacement: 'an aggregate the field\'s type accepts, per '
    ...
    + 'type — `avg` for a `percent`, `count` / `count_distinct` for anything. A DURATION '
    + 'is not recoverable from an aggregate over instants: store it as a number (a '
    + 'computed "days open" field) and aggregate that. A `derived` measure whose `of` '
  ...
  reason:
    ...
    + 'does not exist` (SQLSTATE 42883). ⭐ The silent half is the dangerous one, and it '
    + 'is the DEV default: `derived: { op: \'difference\', of: [avg_a, avg_b] }` over two '
  ...
  acceptanceCriteria:
    'Every dataset measure pairs an `aggregate` with a `field` whose declared type that '
    + 'aggregate accepts. Accepted pairs compile and execute byte-identically to before '
