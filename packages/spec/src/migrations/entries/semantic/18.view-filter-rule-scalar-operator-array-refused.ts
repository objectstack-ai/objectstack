...
export const entry: SemanticMigration = {
  ...
  reason:
    ...
    + 'comparand is refused with the withheld INVALID_FILTER / 400 envelope. The in-memory '
    + 'half is not uniform, and each backend is named rather than generalised: driver-memory '
    + 'REFUSES the same shape in the same envelope (its assertFilterConditionShape throws on '
    + 'an array in the implicit-equality position), while the formula matcher — the one '
    + 'backend that answers the shape at all — excludes every row. So the earlier reading was '
  ...
  acceptanceCriteria:
    ...
    + 'answered 400 INVALID_FILTER on the SQL family and on driver-memory, and the formula '
    + 'matcher, the one backend that answers the shape at all, excluded every row — so '
    + 're-check what the view is supposed to show rather than assuming the old '
