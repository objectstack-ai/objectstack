...
export const FieldSchema = lazySchema(() => {
  ...
  format: z.string().optional().describe('Free-form presentation hint: the spec declares NO vocabulary for it, so any string parses on any field type and nothing validates the value. Two unrelated consumers read it, and which one applies depends on the field type. On an '
    + '`autonumber` field it is the record-number PATTERN — the shorthand that predates `autonumberFormat`, which wins when both are present: `format: \'INV-{0000}\'` mints `INV-0001` on both the query engine and the SQL driver, while a value carrying no `{...}` token is emitted as literal text with the bare counter appended (`format: \'email\'` mints `email1`). Prefer `autonumberFormat` on a new field. '
    + 'On a `date` or `datetime` field the Studio UI grid cell reads it as a display STYLE instead; the style words and the per-type default belong to that renderer and are declared there, and a word it does not recognise is dropped silently onto a default face. '
    + 'Nothing else reads this key — it never affects storage, coercion or write-time validation. To constrain a VALUE, use the field `type` (`email`, `url` and `phone` are field TYPES, not formats) or a `format` validation rule, whose own `format` key is the closed set `email` | `url` | `phone` | `json`.'),
  ...
  expression: EvaluatedExpressionInputSchema.optional().describe('Formula expression (CEL). e.g. F`record.amount * 0.1`'),
  /**
