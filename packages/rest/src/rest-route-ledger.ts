...
export const REST_ROUTE_LEDGER: readonly RestRouteLedgerEntry[] = [
  ...
  { route: 'POST /api/v1/meta/:type/:section/:name/publish', family: 'metadata', source: 'route-manager', disposition: 'sdk', client: 'meta.publishItem',
    note: '[#11932] compound-name arity of the row above, mounted from the same two-entry loop in `registerMetadataEndpoints`. It closes the last hole in the compound-name lifecycle: `PUT /:type/:section/:name?mode=draft` STAGES (#11712/PR #11933), `GET /:type/:section/:name/published` READS (#7526), and until this row there was no per-item door to PROMOTE — writable, readable, and not publishable by the same caller over the same transport. The capability was never missing: `publishMetaItem` keys the draft on type/name/organization/package and reads the name\'s spelling nowhere, measured against the real protocol on a seeded `crm/task` draft before the route existed. `meta.publishItem` already CONSTRUCTED this URL — its own doc comment says "Compound names pass through unencoded, like getItem" — so the SDK promised a door the server had not mounted, and the #3642 URL guard could not see it because the compound name is interpolated into the same `${name}` slot the single-arity pattern declares' },
