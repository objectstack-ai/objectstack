...
describe('ObjectQLStrategy.generateSql — read scope in the previewed WHERE (#3654 residual 2)', () => {
  ...
  it('rejects an OUT-OF-ENVELOPE cross-object preview (cross-object measure)', async () => {
    // An in-envelope cross-object DIMENSION now renders a LEFT JOIN
    // (objectql-crossobj-expand.test.ts); a cross-object MEASURE still cannot be
    // evaluated without a real join, so its preview rejects — /analytics/sql and
    // execute() reject the same set.
    const compiled = compileDataset(
      DatasetSchema.parse({
        name: 'sql_m',
        label: 'x',
    ...
    await expect(
      svc.generateSql({ cube: 'sql_m', dimensions: ['stage'], measures: ['acct_rev'] }, ctxA),
