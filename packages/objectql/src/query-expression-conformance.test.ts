...
const taskObject = {
    ...
    fields: {
        ...
        // column (`SqlDriver.createColumn` → `table.decimal` since #16318, a
        // `table.float` before it) and genuinely
...
describe('#4226 — sort / select / expand on the list path (real ObjectQL engine)', () => {
    ...
    it('a summary field still sorts, in both directions — the family is `formula`, not "computed"', async () => {
        ...
        // (`table.decimal` since #16318, `table.float` before it) and orders
        // correctly — measured on a real SqlDriver
