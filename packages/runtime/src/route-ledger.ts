...
export const ROUTE_LEDGER: readonly RouteLedgerEntry[] = [
  ...
  // ── packages ──────────────────────────────────────────────────────────────
  // [#17431] The `/packages` READ rows carry a `responseSchema` again: the
  // #14242 authoring-vs-assembled manifest stage mismatch that kept `GET
  // /packages` blank through #16781 is declared rather than widened, and both
  // directions are covered in
  // `domains/packages-read-delete-response-conformance.test.ts`. Each row's
  ...
  { route: 'GET /packages/:id', domain: '/packages', disposition: 'sdk', client: 'packages.get',
    ...
    note: 'Same schema shape as the list row above (WHOLE BODY, envelope included) over the same projection — `toPackageResponse` + `withWritableVerdict`, one expression serving two doors. [#17431] bound it to the same two-stage declaration and covered it in `domains/packages-read-delete-response-conformance.test.ts`, which drives THIS handler on both authoring paths. ⚠️ Same measured residue: the door also serves `writable`, which the schema does not carry and a declared parse strips — asserted by name' },
  { route: 'DELETE /packages/:id', domain: '/packages', disposition: 'sdk', client: 'packages.uninstall',
    ...
    note: 'The schema names the WHOLE BODY here, envelope included (`BaseResponseSchema.extend({ data })`), not the `data` alone its lifecycle siblings above declare. Fillable because `domains/packages-read-delete-response-conformance.test.ts` drives THIS handler and parses the payload it answers, on both authoring paths — the row carries no manifest, so the authoring-vs-assembled manifest stage mismatch that kept `GET /packages` blank until [#17431] never reached it. ⚠️ The declaration is a strict SUBSET of the wire: the door also serves `registryRemoved` and `persisted`, which the schema does not carry and a declared parse therefore strips. That residue is asserted by name in the same file rather than fixed — deleting live keys from a published payload is a wire removal, and widening the schema is a `packages/spec` change. See the `GET /packages` row above for the stage mismatch and its ruling' },
  ...
  { route: 'GET /meta/:type/:name/published', domain: '/meta', disposition: 'sdk', client: 'meta.getPublished',
    ...
    note: '[#12038 ruling 1C] enveloped on THIS surface — the named schema is the `data`, and it is DELIBERATELY OPAQUE (`z.unknown()`): the route answers an arbitrary metadata item body, never a union frozen against the type registry. The REST twin (`rest-route-ledger.ts`) answers the same payload BARE' },
