 ...
 *
 * - **Only os:check-marked ts/tsx fences** — the marker is the author's claim
 *   "this is a complete, copyable example", which is exactly the population
 *   whose posture must be clean. Unmarked fragments are not judged (they are
 *   not copyable wholes), matching `check-skill-examples`' own opt-in
 *   boundary. `content/docs/references/` is generated and excluded for the
 *   same reason it is there.
 * - **Two roots, two marker spellings** (#10925, the deferred extension this
 *   header used to record as pending). `skills/` was held back at #10618
 *   because `skills/**` is a governed surface (Prime Directive #14) and its 4
 *   defective examples could not ride a code PR: extending the roots first
 *   would have made this gate born red, the trap #10618's own sequencing note
 *   avoided. Those 4 are fixed in the commit before this one, so the root
 *   lands green. The marker differs PER ROOT and that is not cosmetic —
 *   `.md` carries `<!-- os:check -->` while MDX has no HTML comments
 *   (fumadocs-mdx fails the build outright on `<!-- … -->`), so `.mdx`
 *   carries `{/* os:check *\/}`. `check-skill-examples.ts`'s `SOURCE_ROOTS`
 *   is the precedent this mirrors, down to the per-root exclusions.
...
/**
 * The corpus roots, each with the file extension and marker spelling it owns.
 *
 * `content/docs` was the #10618 scope; `skills` is the #10925 extension (see
 * the header for why it landed second). `references/` is generated by
 * `build-docs.ts` from the schemas, so it cannot drift independently of its
 * source — same exclusion, same reason, as `check-skill-examples`.
 *
 * `floor` is the anti-idle guard, per root: the count of marked blocks and of
 * judged create sites BELOW which the run is a failure rather than a pass. A
 * root that suddenly reads zero has had its extraction rot, not had its prose
 * stop teaching objects (`check-skill-examples` house rule, #4690). The
 * numbers are deliberately well under today's census — they catch extraction
 * breaking, not authors deleting one example.
 */
export const ROOTS = [
  {
    path: 'content/docs',
    ...
    // `<!-- … -->`. Each format's marker follows its own comment syntax.
    marker: '{/* os:check */}',
    exclude: ['content/docs/references'],
    floor: { blocks: 100, sites: 10 },
  },
  {
    path: 'skills',
    label: 'skills',
...
/** Every `ext` file under `root`, sorted, exclusions applied. */
export function proseFiles(rootAbs, excludeAbs, ext) {
  const out = [];
  const walk = (dir) => {
 ...
 * `bodyStartLine + L`.
 *
 * `marker` is the OWNING ROOT's spelling, never both: a block tagged with the
 * other root's form stays unadmitted, exactly as in `check-skill-examples`.
 */
export function markedBlocks(fileAbs, marker) {
 ...
 */
export function judgeFile(fileAbs, relPath, marker) {
  const sites = [];
  const violations = [];
  const refusals = [];
  const notices = [];

  for (const block of markedBlocks(fileAbs, marker)) {
...
/**
 * Scan one prose tree. Parameterized for the self-test; the real run passes
 * each ROOT's own paths, extension, marker and the shared exemption table.
 */
export function scan({ rootAbs, excludeAbs, exemptions, ext = '.mdx', marker = '{/* os:check */}' }) {
  const files = proseFiles(rootAbs, excludeAbs, ext);
  ...
  for (const file of files) {
    ...
    markedBlockCount += markedBlocks(file, marker).length;
    const { sites, violations, refusals, notices } = judgeFile(file, relPath, marker);
...
function main() {
  console.log('🛡  Linting ObjectSchema.create examples in os:check prose blocks (validateSecurityPosture)...\n');

  // An exemption entry is routed to the root that owns its path. One belonging
  // to no root would be dropped by that routing and never judged stale — the
  // ledger would silently stop being self-invalidating, which is the whole
  // discipline. Refuse loudly instead.
  const unclaimed = EXEMPT_SITES.filter((e) => !ROOTS.some((r) => e.file.startsWith(r.path + '/')));
  if (unclaimed.length > 0) {
    fail(
      `exemption entr(ies) under no configured root: ${unclaimed.map((e) => e.file).join(', ')} — ` +
        `an entry no root owns is never judged stale, so the ledger stops self-invalidating`,
    );
  }
  ...
  for (const root of ROOTS) {
    const rootAbs = resolve(REPO_ROOT, root.path);
    if (!existsSync(rootAbs)) {
      fail(`${root.label} root not found: ${root.path} — the corpus moved and this gate must move with it`);
    }
    const { files, markedBlockCount, allSites, gatingViolations, allNotices, errors } = scan({
      rootAbs,
      excludeAbs: root.exclude.map((p) => resolve(REPO_ROOT, p)),
      exemptions: EXEMPT_SITES.filter((e) => e.file.startsWith(root.path + '/')),
      ext: root.ext,
      marker: root.marker,
    });

    // Anti-idle guards, PER ROOT (`check-skill-examples` house rule: a gate
    // that checks nothing must not report success). A root reading below its
    // floor means extraction rotted — a wrong marker spelling above all, which
    // would otherwise leave that whole corpus silently unjudged while the run
    // stayed green on the other root's census.
    if (markedBlockCount < root.floor.blocks) {
      fail(
        `only ${markedBlockCount} \`${root.marker}\` block(s) found under ${root.path} ` +
          `(floor ${root.floor.blocks}) — extraction is broken for the ${root.label} root`,
      );
    }
    if (allSites.length < root.floor.sites && errors.length === 0) {
      fail(
        `only ${allSites.length} ObjectSchema.create site(s) found in marked blocks under ${root.path} ` +
          `(floor ${root.floor.sites}) — the call matcher or evaluator has rotted for the ${root.label} root`,
      );
    }

    for (const n of allNotices) console.log(`   ℹ ${n}`);
    allViolations.push(...gatingViolations);
    allErrors.push(...errors);
    census.push(
      `   ${root.label.padEnd(7)} ${String(allSites.length).padStart(3)} create site(s) in ` +
        `${String(markedBlockCount).padStart(3)} marked block(s) across ${files.length} ${root.ext} file(s)`,
    );
    totalSites += allSites.length;
    totalBlocks += markedBlockCount;
    totalFiles += files.length;
  }

  if (allErrors.length > 0 || allViolations.length > 0) {
    if (allViolations.length > 0) {
      console.error(`\n✗ example(s) carry a security posture \`os validate\` rejects as severity: 'error':\n`);
      for (const v of allViolations) {
  ...
  console.log(
    `\n✅ ${totalSites} ObjectSchema.create example(s) in ${totalBlocks} marked block(s) ` +
      `across ${totalFiles} prose file(s) in ${ROOTS.length} root(s) carry an os validate-clean security posture`,
  );
}
...
function selfTest() {
  ...
  try {
    ...
    // ── The skills root: its own extension AND its own marker (#10925). ─────
    //    Both halves are load-bearing. A `.md` corpus scanned with the MDX
    //    marker reads as zero marked blocks — a whole governed corpus silently
    //    unjudged — which is why the per-root floor above is a failure and not
    //    a shrug, and why this leg asserts the cross-marker misses BOTH ways.
    {
      writeFileSync(
        join(dir, 'skill-red.md'),
        [
          '# A skill', // 1
          '', // 2
          '<!-- os:check -->', // 3
          ...
          `import { ObjectSchema } from '@objectstack/spec/data';`, // 5
          '', // 6
          'export const Case = ObjectSchema.create({', // 7 ← the create call
          `  name: 'support_case',`, // 8
          `  fields: { subject: { type: 'text', required: true } },`, // 9
          '});', // 10
          '```', // 11
          '',
          'Declared posture — silent:',
          '',
          '<!-- os:check -->',
          '```ts',
          `export const Ok = ObjectSchema.create({ name: 'lead', sharingModel: 'private', fields: {} });`,
          '```',
          '',
          'The OTHER root\'s marker spelling must NOT opt a block in here:',
          '',
          '{/* os:check */}',
          '```ts',
          `export const Wrong = ObjectSchema.create({ name: 'wrong_marker_thing', fields: {} });`,
          '```',
          '',
        ].join('\n'),
      );
      const md = scan({ rootAbs: dir, excludeAbs: [], exemptions: [], ext: '.md', marker: '<!-- os:check -->' });
      check(
        md.gatingViolations.length === 1 && md.gatingViolations[0].object === 'support_case',
        `skills: expected exactly the support_case defect, got ${JSON.stringify(md.gatingViolations.map((v) => v.object))} — ` +
          `the .md root must judge its own marker, stay silent on a declared posture, and ignore the MDX spelling`,
      );
      if (md.gatingViolations.length >= 1) {
        check(md.gatingViolations[0].finding.rule === 'security-owd-unset', `skills: expected security-owd-unset, got ${md.gatingViolations[0].finding.rule}`);
        check(md.gatingViolations[0].pageLine === 7, `skills: reported page line ${md.gatingViolations[0].pageLine}, expected 7 — line mapping is wrong for .md`);
      }

      // The .mdx scan must not reach into the .md file at all — extension
      // scoping, not just marker scoping.
      const mdxPass = run();
      check(
        !mdxPass.allSites.some((s) => s.file.endsWith('skill-red.md')),
        `skills: the .mdx root read a .md file — extension scoping is broken`,
      );

      // And the .md root must not admit the docs corpus's own spelling.
      check(
        !md.allSites.some((s) => s.object === 'wrong_marker_thing'),
        `skills: the .md root admitted a block carrying the MDX marker — per-root marker scoping is broken`,
      );
      rmSync(join(dir, 'skill-red.md'));
    }
  ...
  console.log(
    '✅  self-test: flags the measured defect (owd-unset) and the alias value at the right page line;\n' +
      ...
      '    dynamic values and non-ObjectSchema factories; refuses spreads loudly; judges the skills\n' +
      '    root by its own extension AND marker (neither spelling crosses); and holds the exemption\n' +
      '    ledger to the self-invalidating discipline in both directions.',
  );
}
