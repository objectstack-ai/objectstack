...
/**
 * One L2 action body reachable from the stack. Exported (module-level, NOT via
 * the package barrel) because `validate-action-record-writes.ts` checks the
 * OTHER half of the same write surface and must see exactly the same set of
 * bodies: two walks over `actions` + `objects[].actions` would drift, and the
 * dedupe reasoning below is subtle enough that the second copy would drift
 * silently. Same posture as `BODY_WRITE_SYSTEM_FIELDS`.
 */
...
export function collectActionBodies(stack: AnyRec): ActionBodySite[] {
  ...
  const collect = (actions: unknown, pathPrefix: string, parentObject?: string): void => {
    asArray(actions).forEach((action, index) => {
      ...
      const object = actionObjectBinding(action, parentObject);
      const key = `${object ?? ''}\u0000${name}\u0000${source}`;
      if (seen.has(key)) return;
      seen.add(key);
      sites.push({ name, source, object, path: `${pathPrefix}[${index}].body.source` });
    });
  };
