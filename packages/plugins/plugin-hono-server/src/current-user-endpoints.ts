...
/**
 * The permission-map folds `/auth/me/permissions` applies — MOVED to
 * `@objectstack/core` (`security/effective-object-permissions.ts`, #18783) so
 * the endpoint and `ISecurityService.getEffectiveObjectPermissions` compute the
 * `objects` map with ONE function ({@link buildEffectiveObjectPermissions})
 * rather than two copies that agree today. Re-exported here under the same
 * names, so importing them from `@objectstack/plugin-hono-server` is unchanged.
 */
export {
    ...
    rawApp.get(`${prefix}/auth/me/permissions`, withRequestContext(async (c, ctx, resolveCtx) => {
        ...
        try {
            ...
            const resolved: ResolvedPermissionSetLike[] = await resolvePermissionSets(execCtx)
                .catch(() => []);
            // Most-permissive merge of `fields` across all resolved
            // permission sets — same semantics as
            // PermissionEvaluator.getFieldPermissions but for ALL
            // objects in a single pass. (`objects` is merged below, by the
            // one function the security service answers from.)
            ...
            // [#18783] The `objects` slot is the EFFECTIVE object-permission
            // map, and it is computed by the ONE function
            // `ISecurityService.getEffectiveObjectPermissions` answers from —
            // so what the console is told here and the map the server's own
            // `current_user.can()` reads are the same answer, byte for byte,
            // for the same sets. The merge → seed → fold → clamp → annotate
            // order, and why each step exists (ADR-0124 D1/D4, ADR-0092 D2,
            // #3391, #18990), is documented once, on
            // `buildEffectiveObjectPermissions` in `@objectstack/core`.
            const objects = buildEffectiveObjectPermissions(resolved, {
                // The contract's registry view returns `unknown[]` (schema
                // shape is engine-local); narrow to the slice the seed reads.
                allSchemas: () => ql?.registry?.getAllObjects?.() as ApiExposureSchemaLike[] | undefined,
                schemaOf: (name) =>
                    ql?.getSchema?.(name) as (ManagedSchemaLike & ApiExposureSchemaLike) | undefined,
                logger: ctx.logger,
            });
            return c.json({
                authenticated: true,
