 ...
 * string on the page.
 *
 * This set was the LAST of the nine to be wired, and the reason is worth
 * keeping: `withSourceFallback` lives in `@objectstack/platform-objects`, which
 * this package did not declare. It was already in this package's install
 * closure through `@objectstack/service-messaging`, so relying on it without
 * declaring it would have been a phantom dependency; the manifest now declares
 * the resolution that already resolved, and no package joined the graph.
 *
