...
/**
 * [#15302] The `sys_webhook` provenance stamp on a PREDICATE (`multi: true`)
 ...
 * `./webhook-provenance.js` - is a relative import read from source, which is what an
...
async function boot(opts: { stamp: boolean; extraHook?: (engine: any) => void } = { stamp: true }) {
  ...
  if (opts.stamp) bindWebhookProvenanceStamp(engine, silentLogger);
