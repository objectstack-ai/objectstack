...
export const AUTH_ROUTE_LEDGER: readonly AuthRouteLedgerEntry[] = [
  ...
  // #11587 (epic #11586) — the universal org JOIN LINK, V1. better-auth has no
  // link/multi-use invitation concept (verified against the installed 1.7.1;
  // upstream demand tracked at better-auth/better-auth#3672), so these six are
  // ObjectStack mounts over `organization-join-link.ts`, registered ahead of
  // the catch-all. No SDK method builds these URLs — the consumers are the
  // sys_join_link metadata actions and the objectui console dialog +
  // /join/:token landing route (objectui#5961), which post them directly.
  { route: 'POST /api/v1/auth/organization/accept-join-link', family: 'organization', source: 'objectstack', disposition: 'server-only', requires: 'organization', note: 'no SDK method builds this URL — the objectui /join/:token landing route posts it directly; session + VERIFIED email required (GHSA-fmh4-wcc4-5jm3 posture), membership written through the vendor server-only auth.api.addMember with role pinned to member, #11587' },
  { route: 'POST /api/v1/auth/organization/create-join-link', family: 'organization', source: 'objectstack', disposition: 'server-only', requires: 'organization', note: 'no SDK method builds this URL — the sys_join_link create_join_link action and the console dialog post it directly; org owner/admin via the vendor invite-member permission family (invitation:create), one active link per org (409 JOIN_LINK_EXISTS), #11587' },
  { route: 'GET /api/v1/auth/organization/get-join-link', family: 'organization', source: 'objectstack', disposition: 'server-only', requires: 'organization', note: 'no SDK method builds this URL — the console "copy link" read; org owner/admin via the vendor invite-member permission family (invitation:create), returns the live link or null, #11587' },
  { route: 'GET /api/v1/auth/organization/get-join-link-info', family: 'organization', source: 'objectstack', disposition: 'server-only', requires: 'organization', note: 'no SDK method builds this URL — the objectui /join/:token landing page probes it before sign-in; UNAUTHENTICATED by design, rate-limited (tokens are bearer credentials), answers org display name + validity state only and reveals nothing for an unknown token, #11587' },
  { route: 'POST /api/v1/auth/organization/revoke-join-link', family: 'organization', source: 'objectstack', disposition: 'server-only', requires: 'organization', note: 'no SDK method builds this URL — the sys_join_link revoke_join_link action and the console dialog post it directly; org owner/admin via the vendor cancel-invitation permission family (invitation:cancel), #11587' },
  { route: 'POST /api/v1/auth/organization/rotate-join-link', family: 'organization', source: 'objectstack', disposition: 'server-only', requires: 'organization', note: 'no SDK method builds this URL — the sys_join_link rotate_join_link action and the console dialog post it directly; org owner/admin via the vendor invite-member permission family (invitation:create); rotate = revoke current + mint replacement in one request, #11587' },
