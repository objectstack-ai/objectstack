...
/**
 * [#15302] The `sys_email_template` provenance stamp on a PREDICATE (`multi: true`)
 ...
 * `./email-template-provenance.js` - is a relative import read from source, which is what an
...
async function boot(opts: { stamp: boolean; extraHook?: (engine: any) => void } = { stamp: true }) {
  ...
  if (opts.stamp) bindEmailTemplateProvenanceStamp(engine, silentLogger, OBJECT);
  opts.extraHook?.(engine);
  return { engine, driver };
}

const ROWS = (bManagedBy: string) => ([
  { id: 'a', managed_by: 'package', customized: false, subject: 'l' },
  { id: 'b', managed_by: bManagedBy, customized: false, subject: 'l' },
]);

const PAYLOAD = { subject: 'edited' };
...
describe('[#15302] a predicate update stamps EVERY matched row', () => {
  ...
  it('does not stamp when the pre-image is not package/platform managed', async () => {
    ...
    await engine.insert(OBJECT, [
      { id: 'a', managed_by: 'admin', customized: false, subject: 'l' },
      { id: 'b', managed_by: 'user', customized: false, subject: 'l' },
    ]);
    driver.updateManyPayloads.length = 0;
...
describe('[#15302] matched rows that disagree refuse the batch', () => {
  it('refuses with the ADR-0112 envelope and writes nothing', async () => {
    ...
    expect([...driver.store.values()].map((r: any) => [r.customized, r.subject]))
