 ...
 *
 * ```
 * CREATE UNIQUE INDEX uniq_sys_sharing_rule_name
 *   on sys_sharing_rule (name)
 *
 * org_jia POST name=share_west_region  → 201
 ...
 *
 * Sharing rules are authored by admins through the Studio criteria builder, and
 * a rule's whole subject matter — which records of which object are shared with
 * whom — is tenant-local. Two organizations naming a rule `share_west_region`
 * are not in conflict.
 *
 * ⚠️ Note the neighbouring `{ fields: ['organization_id'] }` entry: this object
 * already indexed the tenant column, non-uniquely, while its UNIQUE key ignored
 * it. That is the clearest single illustration of the #4986 trap in the tree.
 *
 ...
 */
describe('sys_sharing_rule — declared uniqueness is organization-scoped (#8554)', () => {
  const uniqueIndexes = (SysSharingRule.indexes ?? []).filter((i: any) => i.unique);
  ...
  it('matches the fixture the driver suite copies, entry for entry', () => {
    ...
    expect(SysSharingRule.indexes).toEqual([
  ...
  it('takes no tenancy opt-out, so organization_id is injected (the scope has a column)', () => {
    ...
    const plan = resolveInjectedSystemColumns(SysSharingRule);
    expect(SysSharingRule.tenancy).toBeUndefined();
