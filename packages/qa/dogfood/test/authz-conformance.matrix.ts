...
export const AUTHZ_CONFORMANCE: AuthzPrimitive[] = [
  ...
  // surface (@objectstack/mcp). MCP is default-ON (isMcpServerEnabled() default
  // true; serve.ts capability push) and agent/external-reachable, so its
  // identity posture is registered here as a CHECKED artifact, not incidental
  // prose in ADR-0096's Evidence table. The probes in authz-conformance.test.ts
  // pin the fail-closed 401 gate AND the buildMcpBridge context threading: drop
  // either and the covers key goes STALE → red CI.
