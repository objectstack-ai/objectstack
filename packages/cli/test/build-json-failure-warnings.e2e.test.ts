...
describe('#11772 — the contract is exhaustive over `compile.ts`, not just over the exits pinned above', () => {
  ...
  it('every `emitJson` exit carries `warnings` — the exit count is READ OFF the source, never an integer', () => {
    ...
    // `validate.ts` pair one field over put `main` in the red with no run to
    // say so (#18848) the moment `validate.ts` gained a seventh, entirely
    // legitimate exit (#18769) — the CONTRACT held throughout and only the
    // COUNT was stale. Because this file is nightly-tier by NAME
    // (`*.e2e.test.ts` — `scripts/nightly-tiers.mjs`), no pull request and no
    // merge-queue run can collect it: measured on the branch that landed this,
    // `OS_TEST_TIERS` unset collects 267 files in this package with ZERO of
    // them this one, and `=nightly` collects 68 with this one in it. An
    // integer that only a cron can read is a pin with no reader at the moment
    // it matters — the honest new exit merges green and `main` is red from
    // that moment until a cron says so.
    //
    ...
    // reviewed anyway: exits being REMOVED. Ten is the population the 2026-08-25
    // ruling in this file's header was made over — the count `#12079` pinned on
    // the day it landed option 1 — not a count of today. (Today is eleven: the
    // tenth FAILURE exit is ADR-0130 D4's per-package author-time rule leg,
    // #14439, a multi-package artifact running the same rule table once per
    // package. It arrived after the ruling and is an exit like any other —
    // which is exactly what a derived count exists to absorb and a frozen one
    // exists to trip over.) It is also the floor under the one vacuum the
    // derived pair shares — an `emitJson` renamed out of existence takes BOTH
    // sides to zero and every assertion here with them.
    expect(
      literals.length,
