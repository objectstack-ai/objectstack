...
describe('client.actions', () => {
    ...
    it('surfaces the handler business failure without throwing (pre-#3913 server: 200 + inner success:false)', async () => {
        // Servers older than #3913 report a handler throw as HTTP 200 with an
        // inner `{ success:false, error }`. A current SDK still has to talk to
        // them, so the inner envelope stays authoritative on a 2xx.
    ...
    it('surfaces a real 4xx as the same non-throwing envelope (#3913)', async () => {
        // Post-#3913 the server answers a handler failure with a real status
        // and `{success:false, error:{message,code}}`. `client.fetch` throws on
        // any non-2xx, but this surface's contract is to REPORT, not throw —
        // callers toast `result.error` rather than wrapping every call in a
        // try/catch.
        const { client } = createMockClient(
            { success: false, error: { message: 'Lead is already converted', code: 'FLOW_FAILED' } },
            400,
        );
        const result = await client.actions.invoke('crm_lead', 'convert', { recordId: 'lead_1' });
        expect(result.success).toBe(false);
    ...
    it('reports a 404 for an unregistered action instead of throwing', async () => {
        const { client } = createMockClient(
