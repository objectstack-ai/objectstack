...
//
// ⚠️ What the floor deliberately does NOT count: the two ledger loops at the end
// of the body walk `WHOLE_SET_ACTIONS` and `ALLOWLIST` and push straight to
// `failures`, one iteration per row. Those are not routed through `expect()` and
// are left unregistered ON PURPOSE — `ALLOWLIST` is an allowlist, so it is meant
// to SHRINK, and a floor that moves with it would red every legitimate removal
// and train the next author to edit the floor, which is the one habit these
// floors exist to prevent (#13797's ruling, carried forward). The floor pins the
// part that does not move with a list: the fixture-tree and refusal cases that
...
const SELF_TEST_BATTERIES = Object.freeze({
  'check-whole-set-label-write self-test': 41,
});
...
export function selfTest() {
  ...
  battery('check-whole-set-label-write self-test');
  const failures = [];
  const silent = () => {};

  const expect = (label, actual, wanted) => {
    // Registered BEFORE the early return, so a passing case counts too — a
    // ledger that only saw failures would read empty on a healthy run.
    registerCase();
