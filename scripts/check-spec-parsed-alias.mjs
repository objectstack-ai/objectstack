...
// comments label individual cases rather than sections, so it carries fewer than
// the two named section banners the sectioning criterion needs, and ⛔ a comment
// is NOT promoted to a section head — that is a judgement per comment this
// transplant does not make. The hoisted single battery is the shape PR #14896
// and PR #15003 landed for exactly this case.
//
...
const SELF_TEST_BATTERIES = Object.freeze({
  'check-spec-parsed-alias self-test': 18,
});
...
function selfTest() {
  ...
  battery('check-spec-parsed-alias self-test');
  const failures = [];
