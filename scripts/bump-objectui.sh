...
#   pnpm objectui:build                       # builds objectui at the NEW pin into .cache/objectui-<SHA12>/
#   node scripts/gen-sdui-manifest-node.mjs   # reads that built tree; no arguments
#
...
# `.objectui-sha`. No browser and no npm install: it enumerates the registry out of
# the pin's BUILT tree, the one producer (ruling 丙 on #17735), and the console build
# ships that same tracked file in its dist. This script prints the reminder on its
# way out; `pnpm objectui:refresh` (this script, then the build) leaves the tree the
# generator needs.
#
...
report_objectui_reachability() {
...

  {
    ...
    echo "    the earliest; until then the committed SDUI manifest regenerated at this pin"
    echo "    describes a registry that is not on main."
    echo
  } >&2
  return 0
}
...
# what this prints:
#
#   pnpm objectui:build                      ->  objectui BUILT at the new pin, in
#                                                .cache/objectui-<SHA12>/
...
#
# `scripts/check-sdui-manifest.mjs` in the required lint job reds while the record's
# pin trails `.objectui-sha`, so only the regeneration turns the bump PR green. It is
# the one producer (ruling 丙 on #17735): it reads the pin's BUILT tree — never an npm
# install, whose version string names a tarball built from an EARLIER commit — and the
# console build ships the same tracked file in packages/console/dist/.
#
# Deliberately a REMINDER, not a hard gate: a machine that cannot build objectui
# must still be able to move the pin, and hard-failing here would be CI cost wearing
# a local disguise. Nothing is lost by that, because the bump PR itself reds in lint
# until the regeneration lands. Printed on BOTH exits below: --no-commit still moved
# the pin.
print_sdui_next_step() {
  echo
  echo "→ NEXT STEP — regenerate the committed SDUI manifest (required lint gate):"
  echo "      pnpm objectui:build                       # skip if objectui:refresh already built this pin"
  echo "      node scripts/gen-sdui-manifest-node.mjs   # reads .cache/objectui-<SHA12>/apps/console"
  echo "  It rewrites the tracked sdui.manifest.json and scripts/sdui-manifest.record.json;"
  echo "  scripts/check-sdui-manifest.mjs reds this bump PR until it does. No browser needed."
