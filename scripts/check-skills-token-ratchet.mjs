 ...
 *      is written by `gen:skill-refs` and `references/react-blocks.md` by
 *      `gen:react-blocks`. A ceiling
...
export const CEILINGS = new Map([
  ...
  // section). Its JSON twin was retired by #14296 item 3 and is gone entirely.
...
// and a sink (`failed++`) that writes only when a case FAILS. Routing THAT
...
// Each row's first column (its label) is a declared battery, verbatim, with a floor of 1, and
...
const SELF_TEST_BATTERIES = Object.freeze({
  ...
  'empty read -> red, not a skip (#4690)': 1,
  'the red names the file': 1,
  'the red names the measured count': 1,
  'the red names the ceiling': 1,
  'the red names the rule: deletion in the same file': 1,
  'the red rejects a re-wrap as payment': 1,
  'shrink direction — a shrunk file is green and its headroom is reported': 1,
  'the gate declares its registry shrink-only in author-facing text': 1,
  'the over-ceiling remedy carries the authority token': 1,
  'the unpriced-file remedy carries the authority token': 1,
  'no remedy tells the author to raise a ceiling': 1,
  'the raise is bound to a quoted maintainer ruling': 1,
  'four ASCII bytes are one token': 1,
  'five ASCII bytes round up to two': 1,
  'the unit is UTF-8 BYTES, not JS characters (a 3-byte CJK char is not free)': 1,
  'empty text counts zero, which the verdict turns red': 1,
  'the catalog is discovered from disk': 1,
  'every discovered file lives under a published skill directory': 1,
  'a discovered file with no ceiling is RED': 1,
  'the unpriced red names the file and its measured count': 1,
  'every discovered AUTHORED file carries a ceiling today': 1,
  'every ceiling names a file the enumeration finds': 1,
  'every ceiling is a positive integer': 1,
  'the walk descends past SKILL.md into rules/': 1,
  'the walk descends arbitrarily deep': 1,
  'a directory carrying no SKILL.md is skipped whole': 1,
  'skills/README.md is outside the population (population 3)': 1,
  'the fixture walk finds exactly its four in-skill files': 1,
  'the real population is more than one file per skill': 1,
  'the real population reaches rules/ and hand-authored references/': 1,
  'a generated _index.md is recognised as generator-owned': 1,
  'the react-blocks contract is recognised as generator-owned': 1,
  'a hand-authored reference is NOT generator-owned': 1,
  'a hand-authored rule is NOT generator-owned': 1,
  'a SKILL.md is NOT generator-owned': 1,
  'a generator-owned file with no ceiling is GREEN, not an unpriced red': 1,
  'its green names the generator that owns it': 1,
  'its green says measured-but-not-ratcheted': 1,
  'a CEILINGS row on a generator-owned file is RED': 1,
  'that red explains the wrong-actor reason': 1,
  'no ceiling names a generator-owned path': 1,
  'the boundary does not swallow the population': 1,
  'the boundary excludes something (it is not inert)': 1,
  'the excluded set is exactly these generator-owned paths': 1,
  'the unpriced red covers a file INSIDE a skill, not just a new skill': 1,
  'the unpriced red routes a genuine generator output to the register, not to a second list': 1,
  'the main basis sha is recorded': 1,
  'the pending-#10402 basis sha is recorded': 1,
  'exactly the two #10402 files are pinned on the second basis': 1,
  'both second-basis files carry a ceiling': 1,
  ...
  'a generator-owned file is counted in the bundle total': 1,
  'a generator-owned file is NOT counted into the ratcheted subtotal': 1,
  'the report separates the generator-owned subtotal': 1,
});
...
// collided.
const SELF_TEST_BATTERY_FLOOR = 64;
...
function selfTest() {
  ...
  const genContract = 'skills/objectstack-ui/references/react-blocks.md';
  ...
  if (failed) {
    console.error(`✗ check-skills-token-ratchet self-test: ${failed} failure(s) (cases and floor).`);
    process.exit(1);
  }
