...
const SELF_TEST_BATTERIES = Object.freeze({
  'the call this gate exists to catch, in each spelling': 4,
  'the OTHER two parser entry points, same gate': 5,
  'the CHECKED calls must not match the banned ones. The names share a': 2,
  'the shared /g regex is reused for a prefilter AND a scan; a stale': 2,
  'the finding is openable': 1,
  'prose and payloads are NOT call sites. Getting this wrong makes the': 5,
  'the sanctioned call is silent': 1,
  'the parser home is exempt, and ONLY the parser home': 3,
  'the out-of-tree census: the SAME scanner, a different verdict': 4,
  'the TIER: which of the three sentences the row is printed under. The': 11,
  'the printed block must not put a row under a sentence that is false of': 2,
  'the masker is load-bearing: prove it, rather than trusting it': 1,
...
export function selfTest() {
  ...
  battery('the call this gate exists to catch, in each spelling');
  ...
  battery('the OTHER two parser entry points, same gate');
  ...
  battery('the CHECKED calls must not match the banned ones. The names share a');
  ...
  battery('the shared /g regex is reused for a prefilter AND a scan; a stale');
  ...
  battery('the finding is openable');
  ...
  battery('prose and payloads are NOT call sites. Getting this wrong makes the');
  ...
  battery('the sanctioned call is silent');
  ...
  battery('the parser home is exempt, and ONLY the parser home');
  ...
  // -- the out-of-tree census: the SAME scanner, a different verdict ---------
  battery('the out-of-tree census: the SAME scanner, a different verdict');
  ...
  battery('the TIER: which of the three sentences the row is printed under. The');
  ...
  battery('the printed block must not put a row under a sentence that is false of');
  ...
  battery('the masker is load-bearing: prove it, rather than trusting it');
