...
const SELF_TEST_BATTERIES = Object.freeze({
  'Control G: the fixture table covers every declared stamp': 1,
  'Control A: a CLEAN corpus is REACHED, and left byte-identical and UNWRITTEN': 20,
  'Control B: a MISSING stamp is a hard failure naming the path': 4,
  'Control C: ONE run names EVERY unstamped surface': 6,
  'Control D: a template with no @objectstack/* dependency exits 1': 2,
...
const SELF_TEST_VERDICT = 'sync-template-versions self-test reached its verdict';
...
function selfTest() {
  ...
  battery('Control G: the fixture table covers every declared stamp');
  assert(
  ...
  try {
    ...
    battery('Control A: a CLEAN corpus is REACHED, and left byte-identical and UNWRITTEN');
    {
    ...
    battery('Control B: a MISSING stamp is a hard failure naming the path');
    {
    ...
    battery('Control C: ONE run names EVERY unstamped surface');
    {
    ...
    battery('Control D: a template with no @objectstack/* dependency exits 1');
    {
    ...
    battery('Control E: a stamp FILE that does not exist exits 1 naming it');
    {
    ...
    battery('Control F: an unparseable template package.json exits 1 naming it');
    {
    ...
    battery('Control H: zero templates refuses a vacuous green');
    {
