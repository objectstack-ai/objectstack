...
//
// The `asserted` counter below is COUNTED and PRINTED, but nothing ever
// compared it: if a case block stopped running the printed number shrank with it
// and the gate stayed green. ⚠️ A verdict line that prints a case count is
// evidence, NOT proof. Closed the way PR #13487 validated on
// check-doc-authoring: what is pinned is the registered NAMES, not a number, and
// the floor requires the OPENED set to equal the DECLARED set with each battery
// at or above its own count.
//
...
const SELF_TEST_BATTERIES = Object.freeze({
  'check-stack-collection-maps self-test': 18,
});
...
function selfTest() {
  ...
  battery('check-stack-collection-maps self-test');
  const failures = [];
