...
const SELF_TEST_BATTERIES = Object.freeze({
  'the approved module may read the probe → GREEN': 1,
  'a raw probe in another package → RED, naming file and line': 1,
  'a COMMENT naming the probe → GREEN (prose explains the rule; it does not break it)': 1,
  'a BLOCK comment is masked, and a real hit AFTER it keeps its line number → RED at :5': 1,
  'a DESTRUCTURED probe → RED': 1,
  'a NAMED IMPORT from node:process → RED': 1,
  'a BRACKETED member access → RED': 1,
  'a .md page discussing the probe is not scanned → GREEN': 1,
  'the approved module missing → RED even though no file breaks the rule': 1,
  'the approved module present but no longer reading the probe → RED': 1,
  'two raw sites → two problems, each named': 1,
});
...
function selfTest() {
  ...
  if (failed > 0) {
    console.error(`\n✗ check-refd-timer-probe self-test: ${failed} failure(s) (cases and floor).`);
    process.exit(1);
  }
