...
// fully green run would register 0 and every battery would read DID NOT RUN,
// the floor inverted rather than installed. So the roster is the table's own
// rows. Each row LABEL is a declared battery, verbatim, with a floor of 1, and
// `registerCase(c.label)` is the FIRST statement of the driving loop body —
// before the `try` whose `catch` ends in `continue`, and before the two guards
// that also end in `continue`. The floor asserts REACH, so a registration
// placed after a guard would reintroduce the very inversion it exists to avoid:
// a row whose guard always skips would read DID NOT RUN, indistinguishable from
// a deleted row. There is no `battery()` opener: for a table-driven self-test
// the ROW is the battery, so attribution is the loop variable rather than a
...
//
// SCOPE, stated so the next reader does not mistake the number: the four
// discovery-block assertions below the loop (`*.md` only, a missing
// `.claude/agents/`, a directory holding no `.md`, and real-tree discovery) are
// NOT rows of this table — they are written inline, one at a time, and the
// verdict line has always counted the 18 ROWS rather than the 22 assertions
// that run. This roster floors the 18 rows; those four assertions are left
// exactly as they are.
const SELF_TEST_BATTERIES = Object.freeze({
  'a definition pinning a tier alias → GREEN': 1,
  'a definition pinning a fully-qualified model id → GREEN': 1,
  'a quoted value → GREEN (both YAML spellings are legal)': 1,
  'a value with a trailing comment → GREEN': 1,
  'R1 — frontmatter with name+description and NO model: → RED': 1,
  'R2 — `model:` present but empty → RED': 1,
  'R3 — no frontmatter at all → RED': 1,
  'R4 — unterminated frontmatter → RED': 1,
  'R5 — a typo of a tier alias → RED (declared-looking, undeclared in effect)': 1,
  'R5b — an alias with a version suffix is not an alias → RED': 1,
  'R6 — `model: inherit` with nothing justifying it → RED': 1,
  'R6b — `model: inherit` WITH a justification entry whose reason is in the file → GREEN': 1,
  'R7 — a justified inherit whose written reason was edited away → RED (self-invalidating)': 1,
  'R8 — a justification entry naming an unscanned file → RED (anti-dormancy)': 1,
  'R9 — a justification entry on a file that now pins a tier → RED (dead config)': 1,
  'R10 — an empty scan → RED, never a green skip (#4690, the whole point)': 1,
  'one bad definition among several → RED, naming the bad one only': 1,
  'an indented `model:` inside description prose does NOT satisfy the rule → RED': 1,
});
...
function selfTest() {
  ...
  if (failed > 0) {
    console.error(`\n✗ check-agent-model-declared self-test: ${failed} failure(s) (cases and floor).`);
    process.exit(1);
  }
