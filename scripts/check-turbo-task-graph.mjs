...
//
// This file declares ONE battery, opened at the top of `selfTest()`'s body. It
// carries ZERO named section banners — fewer than the two the sectioning
// criterion needs — and ⛔ a comment is NOT promoted to a section head: that is
// a judgement per comment this transplant does not make. The hoisted single
// battery is the shape PRs #14896, #15003 and #15217 landed for this case.
//
...
const SELF_TEST_BATTERIES = Object.freeze({
  'check-turbo-task-graph self-test': 49,
});
...
// function. `battery()` opens a battery; every assertion registered after that
// line is attributed to the one most recently opened, so a section that stops
 ...
 * sink `t()` routes through `registerCase()`. It is called from `runSelfTest()`
 * immediately before the success line, so that line can only be printed by a run
 * in which the set of batteries that registered assertions EQUALS the set
 * declared, each at or above its own count. A set difference says WHICH battery
