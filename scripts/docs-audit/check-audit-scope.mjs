...
const SELF_TEST_BATTERIES = Object.freeze({
  'release-owned pages: in scope, read-only (#4920)': 25,
  'the injection contract (#13591)': 1,
});
...
async function selfTest() {
  ...
  battery('release-owned pages: in scope, read-only (#4920)');
  let failed = 0;
  let total = 0;
  ...
  battery('the injection contract (#13591)');
  ...
  const floorFailure = (message) => { console.error(`  ✗ ${message}`); failed++; };
